...
lemma List.pow_le_prod {R : Type*} [Semiring R] [PartialOrder R] [IsStrictOrderedRing R] (l : List R) (d : R)
  ...
  case _ a  l' l_ih =>
    ...
    rw [hk]
    simp only [List.prod_nil, mul_one, length_singleton, pow_one]
    exact h'
...
lemma pow_lt_abs_eval {R K : Type*} [CommRing R] [Field K] (m : R) (i : R →+* K)
  ...
  rw [← Polynomial.eval₂_hom , ← Polynomial.eval_map
    ,Polynomial.Splits.eq_prod_roots (hsplit) ]
  ...
  have aux : d =
    (List.map (⇑abs) (Multiset.map (fun x => i m - x) (map i p).roots).toList).length := by
      ...
      rw [← hdeg, Polynomial.Splits.natDegree_eq_card_roots hsplit]
...
lemma irreducible_of_eval_mul_prime (m : ℤ) (ρ : ℝ) (d p s : ℕ) (hdn : d ≠ 0)
  ...
  have dvdaux : ∀ a , ¬ IsUnit a → a ∣ P → (|m| - ρ) ^ d ≤ |a.eval m| := by
    ...
    have hla : 1 ≤ ‖(map (algebraMap ℤ ℂ) a).leadingCoeff‖ := by
      ...
      simp only [algebraMap_int_eq, eq_intCast, Complex.norm_intCast, ge_iff_le, Eq.symm Int.cast_abs]
      rw [← Int.cast_one, Int.cast_le]
      apply Int.one_le_abs
      exact hazc
    have hrhoaux : ρ ≤ ‖((algebraMap ℤ ℂ) m)‖ := by
      simp only [algebraMap_int_eq, eq_intCast]
      simp_all only [algebraMap_int_eq, Int.cast_abs, norm_intCast]
      linarith
    ...
    convert le_trans (pow_le_pow_right₀ ?_ hadeg) (pow_lt_abs_eval m (algebraMap ℤ ℂ)
      ρ (a.natDegree) hadn (NormedField.toAbsoluteValue ℂ) a ?_ (Polynomial.natDegree_map_eq_of_injective
    (RingHom.injective_int (algebraMap ℤ ℂ)) _ ) hla ha1 hrhoaux)
    · unfold NormedField.toAbsoluteValue
      simp only [Int.cast_abs, algebraMap_int_eq, eq_intCast, AbsoluteValue.coe_mk, MulHom.coe_mk,
        norm_intCast]
    · unfold NormedField.toAbsoluteValue
      dsimp
      simp only [Int.cast_abs, norm_intCast]
    · unfold NormedField.toAbsoluteValue
      dsimp
      convert le_tsub_of_add_le_left hrho
      convert mabs
      simp only [norm_intCast, Int.cast_abs]
...
lemma maxCoeffsAux_nonneg (P : Polynomial ℂ) : 0 ≤ maxCoeffsAux P := by
  ...
  · haveI : NeZero (P.natDegree) := { out := h }
    ...
    use (‖(P.coeff ↑0)‖)
    simp only [norm_nonneg, and_true, List.mem_ofFn]
    use 0
...
lemma coeffs_le_maxCoeffAux (P : Polynomial ℂ ) (n : ℕ) (hn : n < P.natDegree) (h : P.natDegree ≠ 0 ) :
  ...
  unfold maxCoeffsAux
  simp[h]
  refine List.le_maximum_of_mem' (l := List.ofFn (fun n => ‖(P.coeff ↑n)‖ : Fin (P.natDegree) → ℝ)) ?_
...
lemma polynomial_roots_le_cauchy_bound (P : Polynomial ℂ) (z : ℂ)
  ...
  have aux1 : ‖(- (P.eraseLead).eval z)‖≤
    ...
    refine le_trans (AbsoluteValue.sum_le (NormedField.toAbsoluteValue ℂ) _ _) ?_
    ...
    · have aux := Polynomial.eraseLead_natDegree_lt_or_eraseLead_eq_zero P
      ...
      simp only [Finset.range_subset, Order.lt_add_one_iff, Finset.mem_range]
      omega
    · intro i _ _
    ...
    · apply Finset.sum_le_sum
      ...
      unfold NormedField.toAbsoluteValue
      dsimp
      simp only [eraseLead_coeff, ne_of_lt (Finset.mem_range.1 hi), ↓reduceIte, norm_mul, norm_pow,
        le_refl]
  by_cases habs : 0 ≤ ‖z‖ - 1
  · have aux2 : (∑ i ∈ Finset.range (P.natDegree), (‖(P.coeff i)‖) * (‖z‖) ^ i) * (‖z‖ - 1)
      ...
      have aux3 : ∑ i ∈ Finset.range (P.natDegree), (‖(P.coeff i)‖) * (‖z‖) ^ i
        ...
        apply Finset.sum_le_sum
        intro i hi
        refine mul_le_mul_of_nonneg_right ?_ (by rw [← norm_pow] ; exact norm_nonneg (z ^ i) )
    ...
    unfold cauchyBound
    erw [← sub_le_iff_le_add',
      le_div_iff₀ (AbsoluteValue.pos (NormedField.toAbsoluteValue ℂ) (Polynomial.leadingCoeff_ne_zero.2 h)),
...
lemma polynomial_roots_le_cauchy_bound_scale (P : Polynomial ℂ) (z : ℂ)
  ...
  have : r⁻¹ * ‖z‖ = ‖(r⁻¹ * z)‖ := by
    rw [ofReal_inv, norm_mul, norm_inv, Complex.norm_real, Real.norm_eq_abs,
      abs_of_nonneg (a := r) (le_of_lt hs)]
  ...
  simp only [hd, ↓reduceDIte, one_div, natDegree_scaleRoots, Fin.val_cast, coeff_scaleRoots,
    inv_pow]
  congr
  simp only [Complex.norm_mul, norm_inv, norm_pow, norm_real, Real.norm_eq_abs,
    abs_of_nonneg (a := r) (le_of_lt hs)]
...
lemma maxCoeffsAuxScale_ofList (l : List ℤ) (hl : l = l.dropTrailingZeros) (r : ℚ) :
  ...
  · by_cases hlen : l.length - 1 = 0
    ...
    · have hlenc := hlen
      rw [← (natDegree_ofList _ hz hl), add_tsub_cancel_right,
      ← Polynomial.natDegree_map_eq_of_injective (RingHom.injective_int (algebraMap ℤ ℂ))] at hlen
      simp only [maxCoeffsAuxScale, hlen, ↓reduceDIte, coeff_map, eq_intCast, Complex.norm_intCast, one_div,
...
lemma ofList_roots_le_cauchy_bound_scale (l : List ℤ)
  ...
  rw [Polynomial.leadingCoeff_map_of_leadingCoeff_ne_zero, algebraMap_int_eq,
    eq_intCast, Complex.norm_intCast]
