...
lemma IsAdjoinRoot.minpoly_root {Q K : Type*} [Field Q] [CommRing K] [Algebra Q K]
  ...
  let φ := IsAdjoinRoot.algEquiv A (AdjoinRoot.isAdjoinRoot f)
  rw [← minpoly.algEquiv_eq φ (root A), IsAdjoinRoot.algEquiv_root A (AdjoinRoot.isAdjoinRoot f)]
  exact AdjoinRoot.minpoly_root hf

/-- Similar to `Polynomial.Monic.dvd_iff_fraction_map_dvd_fraction_map` but does not assume
  that `p` is monic· -/
lemma Polynomial.Monic_dvd_iff_fraction_map_dvd_fraction_map' {R K : Type*}
  ...
  intro hdvd
  obtain ⟨ k, hk ⟩ := hdvd
  rw [← Polynomial.modByMonic_add_div p] at hk
...
lemma Algebra.minpoly_fraction_field {R Q K : Type*}
  ...
  have heval: (Polynomial.aeval x) T = 0 := by
    rw [Polynomial.aeval_def, ← hcomp, ← Polynomial.eval₂_map, ← hmin,
...
noncomputable def Algebra.adjoin_isAdjoinRoot {R Q K : Type*}
    ...
    refine IsAdjoinRoot.ofAlgEquiv (AdjoinRoot.isAdjoinRoot T) ?_
    ...
    refine AlgEquiv.ofBijective (AdjoinRoot.Minpoly.toAdjoin R x)
      ⟨?_ , AdjoinRoot.Minpoly.toAdjoin.surjective R x⟩
    rw [RingHom.injective_iff_ker_eq_bot, RingHom.ker_eq_bot_iff_eq_zero]
    rintro a ha
    obtain ⟨P, hP⟩ := AdjoinRoot.mk_surjective a
    simp only [← hP, AdjoinRoot.Minpoly.coe_toAdjoin, AdjoinRoot.liftAlgHom_mk,
      toRingHom_ofId, Polynomial.eval₂_eq_eval_map, eval_map_algebraMap, ← Subtype.coe_inj,
      coe_aeval_mk_apply, ZeroMemClass.coe_zero] at ha
    have : aeval x ((map (algebraMap R Q)) P) = 0 := by
      simp only [aeval_map_algebraMap, *]
    have := minpoly.dvd Q x this
    rw [hmin, Polynomial.Monic_dvd_iff_fraction_map_dvd_fraction_map' hm,
      ← AdjoinRoot.mk_eq_zero, ← hmin2 ] at this
    rw [this] at hP
    exact hP.symm
...
noncomputable def Algebra.adjoin_isAdjoinRootOfIsAdjoinRoot {R Q K : Type*}
    [CommRing R] [IsDomain R] [Field Q]
...
noncomputable def Algebra.adjoinRootEquivOfRootMem {R Q K : Type*}
    [CommRing R] [IsDomain R] [Field Q] [Algebra R Q]
  ...
  refine IsAdjoinRoot.algEquiv f (Algebra.adjoin_isAdjoinRoot (Q := Q) T hm k.root ?_)
  rw [IsAdjoinRoot.minpoly_root k (Monic.ne_zero (Monic.map (algebraMap R Q) hm)),
    Monic.map _ hm, inv_one, map_one, mul_one]

lemma surj_fraction_ring_of_eq_rank {R Q K : Type*} {n : ℕ}
    [NeZero n][CommRing R] [IsDomain R] [Field Q] [Algebra R Q]
    [IsFractionRing R Q] [CommRing K] [Algebra R K] [Algebra Q K] [IsScalarTower R Q K]
...
lemma isLocalization_fraction_ring_of_eq_rank {R Q K : Type*} {n : ℕ}
    [NeZero n][CommRing R] [IsDomain R] [Field Q] [Algebra R Q]
    [IsFractionRing R Q] [CommRing K] [Algebra R K] [Algebra Q K] [IsScalarTower R Q K]
    [FiniteDimensional Q K] (hd : Module.finrank Q K = n) (O : Subalgebra R K)
    (b : Basis (Fin n) R O) :
    IsLocalization (Algebra.algebraMapSubmonoid O (nonZeroDivisors R)) K := by
  rw [isLocalization_iff]
  constructor
  · rintro ⟨y, ⟨yp , ⟨hyz, hya⟩ ⟩ ⟩
    refine IsUnit.of_mul_eq_one ((algebraMap Q K) (algebraMap R Q yp)⁻¹) ?_
    dsimp
    simp only [← hya, Subalgebra.coe_algebraMap]
...
lemma discr_eq_discr_fraction_field {R Q K : Type*} {n : ℕ}
    [NeZero n] [CommRing R] [IsDomain R] [Field Q] [Algebra R Q]
    [IsFractionRing R Q] [CommRing K] [Algebra R K] [Algebra Q K] [IsScalarTower R Q K]
    [FiniteDimensional Q K] (hd : Module.finrank Q K = n) (O : Subalgebra R K)
...
lemma discr_of_matrix_mulVec_fraction_field {R Q K : Type*} {n : ℕ}
    [NeZero n] [CommRing R] [IsDomain R] [Field Q] [Algebra R Q]
    [IsFractionRing R Q] [CommRing K] [Algebra R K] [Algebra Q K] [IsScalarTower R Q K]
...
lemma discr_eq_discr_fraction_field_powerBasis {R Q K : Type*} {n : ℕ} [NeZero n] [CommRing R]
    [IsDomain R] [Field Q] [Algebra R Q] [IsFractionRing R Q] [CommRing K] [Algebra R K]
    [Algebra Q K] [IsScalarTower R Q K]
...
noncomputable def Subalgebra.algEquivOfInclusion {R M : Type _} [CommRing R]
    [CommRing M] [Algebra R M] (O Om : Subalgebra R M) (hm : O ≤ Om) :
    O  ≃ₐ[R] (Subalgebra.inclusion hm).range := by
  apply AlgEquiv.ofInjective
  exact inclusion_injective hm

noncomputable def Subalgebra.linearEquivOfInclusion {R M : Type _} [CommRing R]
    [CommRing M] [Algebra R M] (O Om : Subalgebra R M) (hm : O ≤ Om) :
...
lemma inv_det_eq_integral_fraction_field [NeZero n] [CommRing R] [IsDomain R] [Field Q] [Algebra R Q]
    [IsFractionRing R Q] [CommRing K] [Algebra R K] [Algebra Q K] [IsScalarTower R Q K]
    (O O': Subalgebra R K) (hm : O ≤ O') (b : Basis (Fin n) R O) (b' : Basis (Fin n) R O')
    (A : Matrix (Fin n) (Fin n) Q)
    (hb : O'.val ∘ b' = (A.map (algebraMap Q K)).mulVec (O.val ∘ b) ) :
...
theorem isDomainOfIsAdjointRootPrime {O R: Type _} [CommRing O][CommRing R][IsDomain R] [Algebra R O]
  ...
  have := AlgEquiv.toRingEquiv (IsAdjoinRoot.algEquiv hj (AdjoinRoot.isAdjoinRoot T))
  have _ : IsDomain (AdjoinRoot T) := AdjoinRoot.isDomain_of_prime hp
  exact MulEquiv.isDomain (AdjoinRoot T) this
