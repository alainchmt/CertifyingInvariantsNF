import IdealArithmetic.DedekindProject.CertifyRingOfIntegers.CertifyKernel
...
open BigOperators Pointwise Module
...
-- Not exactly the same as LinearMap.frobenius in Mathlib. To do: unify it.
...
lemma smul_top_mem_iff_eq_smul {R M : Type*} [AddCommGroup M][CommRing R][Module R M](y : R) (x : M) :
  ...
  constructor
  · rintro ⟨a, _, ha⟩
    simp only [DistribSMul.toLinearMap_apply] at ha
    exact ⟨a, ha⟩
  · rintro ⟨a, ha⟩
    use a
...
noncomputable def idealSmulNatBasis {O τ R: Type*} [CommRing R] [IsDomain R]
    ...
    [IsTorsionFree R O] (b : Basis τ R O ) :
...
noncomputable def radicalFullRank {O τ R: Type*} [CommRing R] [IsDomain R][IsPrincipalIdealRing R]
    [Fintype τ] [CommRing O] [Algebra R O] [IsTorsionFree R O] (I : Ideal O) (b : Basis τ R O )
    (b' : Basis τ R I):
...
noncomputable def radicalSmulFullRank {O τ R: Type*} [CommRing R] [IsDomain R][IsPrincipalIdealRing R]
    (p : R) (hp : p ≠ 0) [Fintype τ] [CommRing O] [Algebra R O] [IsTorsionFree R O]
...
lemma smul_p_eq_zero (b : M mod' n) :  n • b = 0 := by
  ...
  erw [← hc, ← Submodule.Quotient.mk_smul, Submodule.Quotient.mk_eq_zero,
  ...
  · rintro ⟨y, ⟨m, hyn ⟩ , hy⟩
    ...
    have hmin2 : minpoly (ZMod p) x ∣ f.charpoly := by
      ...
      have :  M f.charpoly := by
        ...
        · intros p' q hp hq
          simp only [M] at ⊢ hp hq
          simp only [ *, aeval_add, LinearMap.add_apply]
        · intros n a
          simp [M, f, Module.algebraMap_end_apply, Algebra.smul_def _ ]
...
noncomputable def basis_radical_of_basis_radical_mod [IsTorsionFree ℤ O]{n m : ℕ}
...
/-- Constructs an explicit basis for `Iₚ / pIₚ` using elements in the kernel of the iterated
  Frobenius map·  -/
noncomputable def basis_radical_of_linear_independent_ker_im [IsTorsionFree ℤ O]
...
lemma basis_radical_of_linear_independent_ker_im_in_b_Ip [IsTorsionFree ℤ O]
...
lemma basis_radical_of_linear_independent_ker_im_def_in_Ip [IsTorsionFree ℤ O]
    {n m j : ℕ} [Nonempty (Fin m ⊕ Fin n)] (b : Fin m → O ) (b' : Fin n → O )
    (v : Fin m → R) (v' : Fin n → R ) (hmod1 : ∀ (i : Fin m), v i = (b i mod'' p))
    (hlin1 : LinearIndependent (ZMod p) v)
    (hlin2 : LinearIndependent (ZMod p) (((Frob R p) ^ j : R →ₗ[ZMod p] R ) ∘ v'))
...
lemma basis_radical_of_linear_independent_ker_im_app [IsTorsionFree ℤ O]
...
lemma basis_radical_of_linear_independent_ker_im_in_Ip [IsTorsionFree ℤ O]
    {n m j : ℕ} [Nonempty (Fin m ⊕ Fin n)]
    (b : Fin m → O ) (b' : Fin n → O ) (v : Fin m → R) (v' : Fin n → R )
    (hmod1 : ∀ (i : Fin m), v i = (b i mod'' p))
    (hlin1 : LinearIndependent (ZMod p) v) (hlin2 : LinearIndependent (ZMod p)
...
lemma basis_radical_of_linear_independent_ker_im_in_repr [IsTorsionFree ℤ O]
    {n m j : ℕ}[Nonempty (Fin m ⊕ Fin n)]
    (b : Fin m → O ) (b' : Fin n → O )(v : Fin m → R) (v' : Fin n → R )
    (hmod1 : ∀ (i : Fin m), v i = (b i mod'' p))
    (hmod2 : ∀ (j : Fin n), v' j = (b' j mod'' p))
  ...
  have aux4 : Finset.univ.sum (λ i => (↑(s i  : ZMod p) • (⟨b i, aux1 i ⟩ mod p ))) +
    ...
    Sum.elim_inr]
    congr
    · simp only [basis_radical_of_linear_independent_ker_im, eq_mpr_eq_cast, cast_eq,
        basis_radical_def_left]
    · simp only [basis_radical_of_linear_independent_ker_im, eq_mpr_eq_cast, cast_eq,
        basis_radical_def_right]
      simp only [zsmul_eq_mul, Int.cast_natCast]
  rw [aux3, aux4]
  symm
...
-- Shortcut instances to avoid timeouts. This is not strictly necessary.
...
lemma mul_ideal_in_p_mul_of_kernel (α : O) :
  ...
  · intros ha x hx
    ...
    simp only [DistribSMul.toLinearMap_apply, natCast_zsmul, Subtype.ext_iff] at hj2
    use j
    rw [← nsmul_eq_mul, ← hj2]
    constructor
...
variable {K : Type*} [CommRing K] [IsTorsionFree ℤ K]
