...
open BigOperators Pointwise Polynomial Module
...
lemma TimesTable.coord_basis_of_equivFun {R M : Type*} {n : ℕ}
  ...
  simp only [Basis.repr_symm_apply, Basis.repr_linearCombination, Finsupp.coe_equivFunOnFinite_symm]
...
lemma subalgebraOfPolys_top_le :
    ⊤ ≤ Submodule.span R (Set.range (λ i => (⟨ h.map (b i) ,
...
lemma linearIndependentOfUpperTriangular [IsFractionRing R Q] :
  ...
  have aux : ∀ i, feq (Ba i) = h.map (map (algebraMap R Q) (B i)) := by
    ...
    simp only [Basis.coe_reindex, OrderIso.coe_symm_toEquiv, Fin.symm_castOrderIso,
      Function.comp_apply, Fin.castOrderIso_apply, IsAdjoinRootMonic.basis_apply, Fin.val_cast,
      Polynomial.map_pow, map_X, map_pow, IsAdjoinRoot.map_X, algebraMap_smul, Ba]
...
lemma toSymmFun_apply_eq {α A: Type*} [LinearOrder α] (f : α → α  → A) (i j : α) :
  ...
  else (this j i (le_of_lt (by grind))).symm
...
lemma basisOfBuilderLists_apply_Fn [DecidableEq R](T : R[X])(l : List R)
  ...
  simp only [Basis.equivFun_symm_apply, AddSubmonoidClass.coe_finset_sum, SetLike.val_smul]
...
variable {O : Type*} [DecidableEq R] {T : R[X]} {l : List R}
...
lemma OfBuilderList_discr_eq_prod_discr' :
  ...
  have : (Oₖ).val ∘ (basisOfBuilderLists T l A) =  Matrix.mulVec (M.map (algebraMap Q K))
    ...
    rw [mul_assoc]
    congr
    unfold AdjM Algebra.adjoin_isAdjoinRootOfIsAdjoinRoot
