...
theorem degree_mul_lt_of_lt_left {p q : R[X]} {a : WithBot ℕ} {b : ℕ}
  ...
  exact (add_comm a ↑b ) ▸ (lt_of_le_of_lt ((add_comm _ (b : WithBot ℕ)) ▸
    add_le_add_left hq p.degree) ((WithBot.add_lt_add_iff_left (x := ↑b) WithBot.coe_ne_bot).2 hp))
...
theorem degree_mul_lt_of_lt_right {p q : R[X]} {a : ℕ} {b : WithBot ℕ}
  ...
  exact degree_mul_lt_of_lt_left hq hp
