...
import IdealArithmetic.Examples.NF5_1_15000000_1.PrimesBelowCert5_1_15000000_1
...
/- Number field `K(α)` with α root of polynomial `X^5 - 10*X^3 - 20*X^2 + 45*X + 108`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, a^2, 1/4*a^3 - 1/2*a^2 - 1/4*a, 1/12*a^4 - 1/12*a^2 - 1/6*a]` -/
...
def SturmRC : SturmBuilderOfList [[108, 45, -20, -10, 0, 1], [45, -40, -30, 0, 5], [-2700, -900, 300, 100], [144000, 16000, -24000], [28800000, 3200000], [34560000000]] [108, 45, -20, -10, 0, 1] [45, -40, -30, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 10000, 576000000, 10240000000000]
  f := [1, 25, 10000, 576000000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [-1500, 500], [-8800000, -2400000], [742400000000, -76800000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[108, 45, -20, -10, 0, 1], [45, -40, -30, 0, 5], [-2700, -900, 300, 100], [144000, 16000, -24000], [28800000, 3200000], [34560000000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 34560000000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 15000000 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (241.10993381773162 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √15000000 ≤ 3872.983356 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √15000000 ≤ 148.732561 := by
