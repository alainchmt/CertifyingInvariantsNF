...
import IdealArithmetic.Examples.NF3_3_658476_1.PrimesBelowCert3_3_658476_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 87*X - 2`.
...
def SturmRC : SturmBuilderOfList [[-2, -87, 0, 1], [-87, 0, 3], [18, 522], [2633904]] [-2, -87, 0, 1] [-87, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-2, -87, 0, 1], [-87, 0, 3], [18, 522], [2633904]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (180.33563138204545 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √658476 ≤ 811.4653509999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √658476 ≤ 180.335634 := by
