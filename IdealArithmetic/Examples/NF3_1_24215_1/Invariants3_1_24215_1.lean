...
import IdealArithmetic.Examples.NF3_1_24215_1.PrimesBelowCert3_1_24215_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 17*X - 13`.
...
def SturmRC : SturmBuilderOfList [[-13, 17, 0, 1], [17, 0, 3], [117, -102], [-24215]] [-13, 17, 0, 1] [17, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-13, 17, 0, 1], [17, 0, 3], [117, -102], [-24215]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (44.03910338860324 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √24215 ≤ 155.611706 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √24215 ≤ 34.590379 := by
