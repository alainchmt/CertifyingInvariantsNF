...
import IdealArithmetic.Examples.NF3_1_337635_1.PrimesBelowCert3_1_337635_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 117*X - 536`.
...
def SturmRC : SturmBuilderOfList [[-536, -117, 0, 1], [-117, 0, 3], [4824, 702], [-1350540]] [-536, -117, 0, 1] [-117, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-536, -117, 0, 1], [-117, 0, 3], [4824, 702], [-1350540]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (164.41739003325017 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √337635 ≤ 581.06369 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √337635 ≤ 129.135264 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 165} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB165

def 𝔭' : Fin 43 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB165.β i ∈ List.ofFn PB165.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB165.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB165) (hc ▸ this) ))
