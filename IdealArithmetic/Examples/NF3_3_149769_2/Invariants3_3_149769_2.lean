import IdealArithmetic.Examples.NF3_3_149769_2.ClassGroupSaturated3_3_149769_2_3
import IdealArithmetic.Examples.NF3_3_149769_2.RelationIdeals3_3_149769_2F0
import IdealArithmetic.Examples.NF3_3_149769_2.RelationIdeals3_3_149769_2F1
import IdealArithmetic.Examples.NF3_3_149769_2.RelationIdeals3_3_149769_2F2
import IdealArithmetic.Examples.NF3_3_149769_2.PrimesBelowCert3_3_149769_2
...
/- Number field `K(α)` with α root of polynomial `X^3 - 129*X - 559`.
Class group structure `[3]`, generated by class of ideals `[(13, a - 5)]`. -/
...
def SturmRC : SturmBuilderOfList [[-559, -129, 0, 1], [-129, 0, 3], [5031, 774], [149769]] [-559, -129, 0, 1] [-129, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-559, -129, 0, 1], [-129, 0, 3], [5031, 774], [149769]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
def NPSU3 : pSaturatedClassGroupCertificateNDvdT 3 ![J0] ![3] where 
 ...
 hDl := by decide 
 NInv := ![![1, 1, 2], ![1, 0, 1], ![2, 1, 2]]
 hN := by decide 

lemma T_discr : T.discr = 149769 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 149769 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (86.01 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √149769 ≤ 387.00001 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √149769 ≤ 86.010002 := by
...
def 𝔭' : Fin 9 → nonZeroDivisors (Ideal O) := by
  intro i
...
lemma 𝔭'_apply : ∀ (i : Fin 9), ↑(𝔭' i) = (Fin.addCasesIter e 𝔭) i := by
  intro i
  rw [← cert_eq_𝔭]
  rfl
...
lemma relations_proof0 (i : Fin 5) : IsInClass O J (𝔭 0 i) (BM 0 i) := by
  show IsInClass O J (𝔭 0 i) (![![0], ![2], ![1], ![1], ![1]] i)
  unfold IsInClass
...
lemma relations_proof1 (i : Fin 1) : IsInClass O J (𝔭 1 i) (BM 1 i) := by
  show IsInClass O J (𝔭 1 i) (![![2]] i)
  unfold IsInClass
  simp only [Fin.isValue, Fin.prod_univ_castSucc, Finset.univ_eq_empty, Finset.prod_empty, Fin.succ_zero_eq_one, one_mul]
  fin_cases i
