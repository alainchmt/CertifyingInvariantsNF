...
import IdealArithmetic.Examples.NF3_1_80359_1.PrimesBelowCert3_1_80359_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 20*X - 387`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/7*a^2 + 1/7*a + 1/7]` -/
...
def SturmRC : SturmBuilderOfList [[-387, 20, -1, 1], [20, -2, 3], [3463, -118], [-3937591]] [-387, 20, -1, 1] [20, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-387, 20, -1, 1], [20, -2, 3], [3463, -118], [-3937591]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (80.21747908263792 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √80359 ≤ 283.47664 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √80359 ≤ 63.004809 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 81} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB81

def 𝔭' : Fin 24 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB81.β i ∈ List.ofFn PB81.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB81.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB81) (hc ▸ this) ))
