import IdealArithmetic.Examples.NF3_1_83820_3.ClassGroupSaturated3_1_83820_3_3
import IdealArithmetic.Examples.NF3_1_83820_3.RelationIdeals3_1_83820_3F0
import IdealArithmetic.Examples.NF3_1_83820_3.RelationIdeals3_1_83820_3F1
import IdealArithmetic.Examples.NF3_1_83820_3.PrimesBelowCert3_1_83820_3
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 45*X + 177`.
...
def SturmRC : SturmBuilderOfList [[177, -45, -1, 1], [-45, -2, 3], [-1548, 272], [-335280]] [177, -45, -1, 1] [-45, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[177, -45, -1, 1], [-45, -2, 3], [-1548, 272], [-335280]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (81.92650847546221 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √83820 ≤ 289.516849 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √83820 ≤ 64.347078 := by
...
def BM : (i : Fin 2) → Fin (e i) → (Fin 1 → ℕ) := by 
  rintro ⟨i, hi⟩ 
  interval_cases i 
  exact ![![2], ![1], ![1], ![1], ![1], ![0], ![0], ![1], ![1], ![1], ![2], ![0], ![1], ![1], ![1], ![2]]
  exact ![![1], ![2], ![2], ![2], ![1], ![0], ![0], ![0], ![0]]

lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 82} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB82

def 𝔭' : Fin 25 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB82.β i ∈ List.ofFn PB82.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB82.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB82) (hc ▸ this) ))
...
lemma relations_proof0 (i : Fin 16) : IsInClass O J (𝔭 0 i) (BM 0 i) := by
  show IsInClass O J (𝔭 0 i) (![![2], ![1], ![1], ![1], ![1], ![0], ![0], ![1], ![1], ![1], ![2], ![0], ![1], ![1], ![1], ![2]] i)
  unfold IsInClass
...
lemma relations_proof1 (i : Fin 9) : IsInClass O J (𝔭 1 i) (BM 1 i) := by
  show IsInClass O J (𝔭 1 i) (![![1], ![2], ![2], ![2], ![1], ![0], ![0], ![0], ![0]] i)
  unfold IsInClass
  simp only [Fin.isValue, Fin.prod_univ_castSucc, Finset.univ_eq_empty, Finset.prod_empty, Fin.succ_zero_eq_one, one_mul]
  fin_cases i
