...
import IdealArithmetic.Examples.NF3_1_196911_1.PrimesBelowCert3_1_196911_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 3*X - 427`.
...
def SturmRC : SturmBuilderOfList [[-427, -3, 0, 1], [-3, 0, 3], [3843, 18], [-4922775]] [-427, -3, 0, 1] [-3, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-427, -3, 0, 1], [-3, 0, 3], [3843, 18], [-4922775]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (125.56458986357083 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √196911 ≤ 443.746559 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √196911 ≤ 98.620346 := by
