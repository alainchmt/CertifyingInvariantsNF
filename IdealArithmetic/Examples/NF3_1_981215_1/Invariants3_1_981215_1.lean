...
import IdealArithmetic.Examples.NF3_1_981215_1.PrimesBelowCert3_1_981215_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 94*X - 2099`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/11*a^2 - 3/11*a + 1/11]` -/
...
def SturmRC : SturmBuilderOfList [[-2099, 94, -1, 1], [94, -2, 3], [18797, -562], [-118727015]] [-2099, 94, -1, 1] [94, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-2099, 94, -1, 1], [94, -2, 3], [18797, -562], [-118727015]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -118727015 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -981215 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (280.2819881196415 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √981215 ≤ 990.5629809999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √981215 ≤ 220.135107 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 281} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB281

def 𝔭' : Fin 68 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB281.β i ∈ List.ofFn PB281.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB281.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB281) (hc ▸ this) ))
