...
import IdealArithmetic.Examples.NF3_1_107543_1.PrimesBelowCert3_1_107543_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 44*X + 413`.
...
def SturmRC : SturmBuilderOfList [[413, 44, -1, 1], [44, -2, 3], [-3761, -262], [-5269607]] [413, 44, -1, 1] [44, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[413, 44, -1, 1], [44, -2, 3], [-3761, -262], [-5269607]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (92.79733013743274 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √107543 ≤ 327.937504 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √107543 ≤ 72.885001 := by
