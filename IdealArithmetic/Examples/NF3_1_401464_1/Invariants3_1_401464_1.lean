...
import IdealArithmetic.Examples.NF3_1_401464_1.PrimesBelowCert3_1_401464_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 13*X + 239`.
...
def SturmRC : SturmBuilderOfList [[239, 13, -1, 1], [13, -2, 3], [-2164, -76], [-1605856]] [239, 13, -1, 1] [13, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[239, 13, -1, 1], [13, -2, 3], [-2164, -76], [-1605856]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (179.28548601869554 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √401464 ≤ 633.6118789999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √401464 ≤ 140.81264 := by
