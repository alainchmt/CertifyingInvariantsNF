...
import IdealArithmetic.Examples.NF3_3_701244_1.PrimesBelowCert3_3_701244_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 114*X - 340`.
...
def SturmRC : SturmBuilderOfList [[-340, -114, 0, 1], [-114, 0, 3], [3060, 684], [2804976]] [-340, -114, 0, 1] [-114, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-340, -114, 0, 1], [-114, 0, 3], [3060, 684], [2804976]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (186.09958416132088 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √701244 ≤ 837.403139 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √701244 ≤ 186.099586 := by
