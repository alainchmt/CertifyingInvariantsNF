...
import IdealArithmetic.Examples.NF3_3_271377_1.PrimesBelowCert3_3_271377_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 69*X - 92`.
...
def SturmRC : SturmBuilderOfList [[-92, -69, 0, 1], [-69, 0, 3], [828, 414], [1085508]] [-92, -69, 0, 1] [-69, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-92, -69, 0, 1], [-69, 0, 3], [828, 414], [1085508]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (115.77412800748483 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √271377 ≤ 520.938586 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √271377 ≤ 115.77413 := by
