...
import IdealArithmetic.Examples.NF3_1_35075_1.PrimesBelowCert3_1_35075_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 22*X - 68`.
...
def SturmRC : SturmBuilderOfList [[-68, 22, -1, 1], [22, -2, 3], [590, -130], [-140300]] [-68, 22, -1, 1] [22, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-68, 22, -1, 1], [22, -2, 3], [590, -130], [-140300]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (53.00030810991945 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √35075 ≤ 187.283218 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √35075 ≤ 41.628493 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 54} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB54

def 𝔭' : Fin 15 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB54.β i ∈ List.ofFn PB54.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB54.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB54) (hc ▸ this) ))
