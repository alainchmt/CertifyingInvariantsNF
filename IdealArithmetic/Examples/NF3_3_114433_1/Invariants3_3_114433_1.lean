...
import IdealArithmetic.Examples.NF3_3_114433_1.PrimesBelowCert3_3_114433_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 32*X - 17`.
...
def SturmRC : SturmBuilderOfList [[-17, -32, -1, 1], [-32, -2, 3], [185, 194], [114433]] [-17, -32, -1, 1] [-32, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-17, -32, -1, 1], [-32, -2, 3], [185, 194], [114433]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (75.18321561353494 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √114433 ≤ 338.27948 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √114433 ≤ 75.183218 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 76} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB76

def 𝔭' : Fin 27 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB76.β i ∈ List.ofFn PB76.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB76.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB76) (hc ▸ this) ))
