...
import IdealArithmetic.Examples.NF5_1_25312500_2.PrimesBelowCert5_1_25312500_2
...
/- Number field `K(α)` with α root of polynomial `X^5 - 30*X^2 + 15*X + 78`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/2*a^2 - 1/2*a, 1/4*a^3 + 1/4*a - 1/2, 1/8*a^4 - 1/8*a^3 + 1/8*a^2 + 1/8*a - 1/4]` -/
...
def SturmRC : SturmBuilderOfList [[78, 15, -30, 0, 0, 1], [15, -60, 0, 0, 5], [-1950, -300, 450], [-86400000, 21600000], [-103680000000]] [78, 15, -30, 0, 0, 1] [15, -60, 0, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 91125000, 466560000000000]
  f := [1, 125, 18225000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [4837500, 675000, 1012500], [32400000000, 9720000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[78, 15, -30, 0, 0, 1], [15, -60, 0, 0, 5], [-1950, -300, 450], [-86400000, 21600000], [-103680000000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
def 𝔭' : Fin 69 → nonZeroDivisors (Ideal O) := by
  intro i
...
lemma 𝔭'_apply : ∀ (i : Fin 69), ↑(𝔭' i) = (Fin.addCasesIter e 𝔭) i := by
  intro i
  rw [← cert_eq_𝔭]
  rfl
