...
import IdealArithmetic.Examples.NF3_1_132020_1.PrimesBelowCert3_1_132020_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 70`.
...
def SturmRC : SturmBuilderOfList [[70, 0, -1, 1], [0, -2, 3], [-630, 2], [-132020]] [70, 0, -1, 1] [0, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (102.81576498976126 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √132020 ≤ 363.345576 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √132020 ≤ 80.753461 := by
