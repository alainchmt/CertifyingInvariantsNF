...
import IdealArithmetic.Examples.NF3_1_342171_1.PrimesBelowCert3_1_342171_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 102*X - 681`.
...
def SturmRC : SturmBuilderOfList [[-681, 102, 0, 1], [102, 0, 3], [6129, -612], [-16766379]] [-681, 102, 0, 1] [102, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-681, 102, 0, 1], [102, 0, 3], [6129, -612], [-16766379]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -16766379 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -342171 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (165.51808143326232 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √342171 ≤ 584.953854 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √342171 ≤ 129.999745 := by
