...
import IdealArithmetic.Examples.NF5_1_16200000_1.PrimesBelowCert5_1_16200000_1
...
/- Number field `K(α)` with α root of polynomial `X^5 - 20*X^2 - 45*X - 96`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, a^2, 1/2*a^3 - 1/2*a, 1/48*a^4 + 5/48*a^3 - 23/48*a^2 + 3/16*a]` -/
...
def SturmRC : SturmBuilderOfList [[-96, -45, -20, 0, 0, 1], [-45, -40, 0, 0, 5], [2400, 900, 300], [18360000, -14040000], [-149299200000]] [-96, -45, -20, 0, 0, 1] [-45, -40, 0, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 27000000, 197121600000000]
  f := [1, 125, 5400000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [450000, -1350000, 450000], [-18144000000, -4212000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-96, -45, -20, 0, 0, 1], [-45, -40, 0, 0, 5], [2400, 900, 300], [18360000, -14040000], [-149299200000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 149299200000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 16200000 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (250.56840104428295 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √16200000 ≤ 4024.9223690000003 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √16200000 ≤ 154.567019 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 251} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB251

def 𝔭' : Fin 52 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB251.β i ∈ List.ofFn PB251.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB251.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB251) (hc ▸ this) ))
