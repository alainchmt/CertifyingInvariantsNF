...
import IdealArithmetic.Examples.NF5_1_3515625_1.PrimesBelowCert5_1_3515625_1
...
/- Number field `K(α)` with α root of polynomial `X^5 + 10*X^3 - 15*X^2 + 10*X - 12`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, a^2, a^3, 1/22*a^4 + 2/11*a^3 + 2/11*a^2 + 1/22*a - 4/11]` -/
...
theorem O_integral_closure : O = integralClosure ℤ K := by
  ...
  · haveI : Fact $ Nat.Prime p := fact_iff.2 hp
    refine piMaximal_of_root_in_order_of_satisfiesDedekindCriterion_int Adj T_monic hm ?_ hroot_mem
     (satisfiesDedekindAlmostAllLists_of_certificate T _ T_ofList [2, 11] D p hp hc)
...
def SturmRC : SturmBuilderOfList [[-12, 10, -15, 10, 0, 1], [10, -30, 30, 0, 5], [300, -200, 225, -100], [-17500, 15000, -18125], [-5343750, -468750], [1701562500]] [-12, 10, -15, 10, 0, 1] [10, -30, 30, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 10000, 328515625, 219726562500]
  f := [1, 25, 10000, 328515625]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [-1125, -500], [-2578125, 1812500], [-103886718750, 8496093750]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-12, 10, -15, 10, 0, 1], [10, -30, 30, 0, 5], [300, -200, 225, -100], [-17500, 15000, -18125], [-5343750, -468750], [1701562500]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 1701562500 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 3515625 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (116.73200355597312 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √3515625 ≤ 1875.00001 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √3515625 ≤ 72.01 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 117} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB117

def 𝔭' : Fin 21 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB117.β i ∈ List.ofFn PB117.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB117.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB117) (hc ▸ this) ))
