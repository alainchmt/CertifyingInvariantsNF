...
import IdealArithmetic.Examples.NF5_1_9000000_1.PrimesBelowCert5_1_9000000_1
...
/- Number field `K(α)` with α root of polynomial `X^5 + 10*X^3 + 40*X - 96`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/2*a^2, 1/4*a^3 - 1/2*a, 1/24*a^4 - 1/12*a^3 + 1/12*a^2 - 1/6*a]` -/
...
def SturmRC : SturmBuilderOfList [[-96, 40, 0, 10, 0, 1], [40, 0, 30, 0, 5], [2400, -800, 0, -100], [-16000, -48000, 4000], [3840000, 24960000], [331776000000]] [-96, 40, 0, 10, 0, 1] [40, 0, 30, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 10000, 16000000, 623001600000000]
  f := [1, 25, 10000, 16000000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [0, -500], [-4800000, -400000], [-1213440000000, 99840000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-96, 40, 0, 10, 0, 1], [40, 0, 30, 0, 5], [2400, -800, 0, -100], [-16000, -48000, 4000], [3840000, 24960000], [331776000000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 331776000000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 9000000 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (186.76520568955698 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √9000000 ≤ 3000.00001 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √9000000 ≤ 115.21 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 187} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB187

def 𝔭' : Fin 41 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB187.β i ∈ List.ofFn PB187.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB187.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB187) (hc ▸ this) ))
...
lemma relations_proof1 (i : Fin 3) : IsInClass O J (𝔭 1 i) (BM 1 i) := by
  show IsInClass O J (𝔭 1 i) (![![0], ![0], ![0]] i)
  unfold IsInClass
  fin_cases i
  · refine ideal_mem_principal_class''' (by decide) J rfl rfl
  · refine ideal_mem_principal_class''' (by decide) J rfl rfl
