...
import IdealArithmetic.Examples.NF3_1_383075_1.PrimesBelowCert3_1_383075_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 142*X + 252`.
...
def SturmRC : SturmBuilderOfList [[252, 142, -1, 1], [142, -2, 3], [-2410, -850], [-13790700]] [252, 142, -1, 1] [142, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[252, 142, -1, 1], [142, -2, 3], [-2410, -850], [-13790700]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -13790700 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -383075 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (175.13151672137806 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √383075 ≤ 618.930539 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √383075 ≤ 137.55012 := by
