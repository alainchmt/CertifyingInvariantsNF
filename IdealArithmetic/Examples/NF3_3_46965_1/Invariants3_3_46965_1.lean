...
import IdealArithmetic.Examples.NF3_3_46965_1.PrimesBelowCert3_3_46965_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 48*X - 27`.
...
def SturmRC : SturmBuilderOfList [[-27, -48, 0, 1], [-48, 0, 3], [243, 288], [422685]] [-27, -48, 0, 1] [-48, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-27, -48, 0, 1], [-48, 0, 3], [243, 288], [422685]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (48.16868830501158 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √46965 ≤ 216.714107 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √46965 ≤ 48.16869 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 49} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB49

def 𝔭' : Fin 15 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB49.β i ∈ List.ofFn PB49.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB49.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB49) (hc ▸ this) ))
...
lemma 𝔭'_apply : ∀ (i : Fin 15), ↑(𝔭' i) = (Fin.addCasesIter e 𝔭) i := by
  intro i
  rw [← cert_eq_𝔭]
  rfl
