...
import IdealArithmetic.Examples.NF3_3_564980_1.PrimesBelowCert3_3_564980_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 85*X - 63`.
...
def SturmRC : SturmBuilderOfList [[-63, -85, -1, 1], [-85, -2, 3], [652, 512], [2259920]] [-63, -85, -1, 1] [-85, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-63, -85, -1, 1], [-85, -2, 3], [652, 512], [2259920]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (167.04366999973462 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √564980 ≤ 751.651525 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √564980 ≤ 167.043672 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 168} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB168

def 𝔭' : Fin 41 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB168.β i ∈ List.ofFn PB168.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB168.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB168) (hc ▸ this) ))
