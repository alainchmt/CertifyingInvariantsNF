...
import IdealArithmetic.Examples.NF3_3_783380_1.PrimesBelowCert3_3_783380_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 170*X - 50`.
...
def SturmRC : SturmBuilderOfList [[-50, -170, 0, 1], [-170, 0, 3], [450, 1020], [19584500]] [-50, -170, 0, 1] [-170, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-50, -170, 0, 1], [-170, 0, 3], [450, 1020], [19584500]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 19584500 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 783380 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (196.6961258420772 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √783380 ≤ 885.087576 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √783380 ≤ 196.696128 := by
