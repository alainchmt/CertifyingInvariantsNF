...
import IdealArithmetic.Examples.NF3_1_225228_1.PrimesBelowCert3_1_225228_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 548`.
...
def SturmRC : SturmBuilderOfList [[-548, 0, 0, 1], [0, 0, 3], [4932]] [-548, 0, 0, 1] [0, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (134.28921541828535 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √225228 ≤ 474.581931 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √225228 ≤ 105.472651 := by
