...
import IdealArithmetic.Examples.NF3_1_334235_1.PrimesBelowCert3_1_334235_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 55*X + 212`.
...
def SturmRC : SturmBuilderOfList [[212, -55, -1, 1], [-55, -2, 3], [-1853, 332], [-334235]] [212, -55, -1, 1] [-55, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[212, -55, -1, 1], [-55, -2, 3], [-1853, 332], [-334235]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (163.5875004956439 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √334235 ≤ 578.130618 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √334235 ≤ 128.483471 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 164} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB164

def 𝔭' : Fin 48 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB164.β i ∈ List.ofFn PB164.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB164.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB164) (hc ▸ this) ))
