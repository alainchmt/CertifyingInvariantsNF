...
import IdealArithmetic.Examples.NF3_1_735727_1.PrimesBelowCert3_1_735727_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 55*X - 51`.
...
def SturmRC : SturmBuilderOfList [[-51, 55, 0, 1], [55, 0, 3], [459, -330], [-735727]] [-51, 55, 0, 1] [55, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-51, 55, 0, 1], [55, 0, 3], [459, -330], [-735727]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (242.70227455868908 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √735727 ≤ 857.74531 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √735727 ≤ 190.620069 := by
