...
import IdealArithmetic.Examples.NF3_1_29535_1.PrimesBelowCert3_1_29535_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 33`.
...
def SturmRC : SturmBuilderOfList [[-33, 0, -1, 1], [0, -2, 3], [297, 2], [-29535]] [-33, 0, -1, 1] [0, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (48.63572531663711 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √29535 ≤ 171.857509 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √29535 ≤ 38.200558 := by
