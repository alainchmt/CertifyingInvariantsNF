...
import IdealArithmetic.Examples.NF3_1_482727_1.PrimesBelowCert3_1_482727_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 154*X + 903`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/9*a^2 + 1/9*a + 1/3]` -/
...
def SturmRC : SturmBuilderOfList [[903, 154, -1, 1], [154, -2, 3], [-8281, -922], [-39100887]] [903, 154, -1, 1] [154, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[903, 154, -1, 1], [154, -2, 3], [-8281, -922], [-39100887]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -39100887 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -482727 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (196.59410523866305 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √482727 ≤ 694.785588 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √482727 ≤ 154.406797 := by
