...
import IdealArithmetic.Examples.NF3_1_871832_1.PrimesBelowCert3_1_871832_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 259*X - 1758`.
...
def SturmRC : SturmBuilderOfList [[-1758, -259, 0, 1], [-259, 0, 3], [15822, 1554], [-13949312]] [-1758, -259, 0, 1] [-259, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-1758, -259, 0, 1], [-259, 0, 3], [15822, 1554], [-13949312]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -13949312 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -871832 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (264.1985599373881 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √871832 ≤ 933.7194539999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √871832 ≤ 207.503212 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 265} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB265

def 𝔭' : Fin 53 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB265.β i ∈ List.ofFn PB265.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB265.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB265) (hc ▸ this) ))
