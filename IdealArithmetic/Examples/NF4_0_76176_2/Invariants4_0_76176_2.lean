...
theorem K_minowski : minkowskiBoundFB K ≤ (41.95697002792784 : ℝ) := by
  ...
  have hraux: √76176 ≤ 276.00001 := by
      ...
      norm_num
  have : 0.09375 * √76176 ≤ 25.885001 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 42} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB42

def 𝔭' : Fin 12 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB42.β i ∈ List.ofFn PB42.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB42.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB42) (hc ▸ this) ))
...
lemma 𝔭'_apply : ∀ (i : Fin 12), ↑(𝔭' i) = (Fin.addCasesIter e 𝔭) i := by
  intro i
  rw [← cert_eq_𝔭]
  rfl
...
lemma relations_proof0 (i : Fin 12) : IsInClass O J (𝔭 0 i) (BM 0 i) := by
  show IsInClass O J (𝔭 0 i) (![![5, 0], ![1, 0], ![1, 1], ![5, 1], ![0, 1], ![0, 1], ![2, 1], ![4, 1], ![2, 1], ![4, 1], ![3, 1], ![3, 1]] i)
  unfold IsInClass
