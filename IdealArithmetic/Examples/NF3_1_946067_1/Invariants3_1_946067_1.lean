...
import IdealArithmetic.Examples.NF3_1_946067_1.PrimesBelowCert3_1_946067_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 40*X - 211`.
...
def SturmRC : SturmBuilderOfList [[-211, -40, 0, 1], [-40, 0, 3], [1899, 240], [-946067]] [-211, -40, 0, 1] [-40, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-211, -40, 0, 1], [-40, 0, 3], [1899, 240], [-946067]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (275.2164142878034 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √946067 ≤ 972.659766 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √946067 ≤ 216.156615 := by
