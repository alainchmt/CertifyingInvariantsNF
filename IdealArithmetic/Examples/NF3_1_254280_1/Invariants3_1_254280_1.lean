...
import IdealArithmetic.Examples.NF3_1_254280_1.PrimesBelowCert3_1_254280_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 34*X + 270`.
...
def SturmRC : SturmBuilderOfList [[270, 34, -1, 1], [34, -2, 3], [-2464, -202], [-2288520]] [270, 34, -1, 1] [34, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[270, 34, -1, 1], [34, -2, 3], [-2464, -202], [-2288520]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (142.68691365505586 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √254280 ≤ 504.261847 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √254280 ≤ 112.068188 := by
