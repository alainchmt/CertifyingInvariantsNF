...
import IdealArithmetic.Examples.NF3_1_391287_1.PrimesBelowCert3_1_391287_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 9*X - 361`.
...
def SturmRC : SturmBuilderOfList [[-361, 9, 0, 1], [9, 0, 3], [3249, -54], [-3521583]] [-361, 9, 0, 1] [9, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-361, 9, 0, 1], [9, 0, 3], [3249, -54], [-3521583]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (176.99860837189436 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √391287 ≤ 625.5293859999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √391287 ≤ 139.01653 := by
