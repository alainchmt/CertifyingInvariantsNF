...
import IdealArithmetic.Examples.NF3_3_104253_1.PrimesBelowCert3_3_104253_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 75*X - 144`.
...
def SturmRC : SturmBuilderOfList [[-144, -75, -1, 1], [-75, -2, 3], [1371, 452], [938277]] [-144, -75, -1, 1] [-75, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-144, -75, -1, 1], [-75, -2, 3], [1371, 452], [938277]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (71.76162922398555 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √104253 ≤ 322.882342 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √104253 ≤ 71.761632 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 72} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB72

def 𝔭' : Fin 21 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB72.β i ∈ List.ofFn PB72.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB72.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB72) (hc ▸ this) ))
