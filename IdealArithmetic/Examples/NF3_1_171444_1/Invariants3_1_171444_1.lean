...
import IdealArithmetic.Examples.NF3_1_171444_1.PrimesBelowCert3_1_171444_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 9*X - 478`.
...
def SturmRC : SturmBuilderOfList [[-478, 9, 0, 1], [9, 0, 3], [4302, -54], [-6171984]] [-478, 9, 0, 1] [9, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-478, 9, 0, 1], [9, 0, 3], [4302, -54], [-6171984]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (117.16443940920675 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √171444 ≤ 414.057977 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √171444 ≤ 92.022884 := by
