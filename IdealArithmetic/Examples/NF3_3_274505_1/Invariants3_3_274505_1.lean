...
import IdealArithmetic.Examples.NF3_3_274505_1.PrimesBelowCert3_3_274505_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 56*X - 109`.
...
def SturmRC : SturmBuilderOfList [[-109, -56, -1, 1], [-56, -2, 3], [1037, 338], [274505]] [-109, -56, -1, 1] [-56, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-109, -56, -1, 1], [-56, -2, 3], [1037, 338], [274505]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (116.43938833961039 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √274505 ≤ 523.9322579999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √274505 ≤ 116.439391 := by
