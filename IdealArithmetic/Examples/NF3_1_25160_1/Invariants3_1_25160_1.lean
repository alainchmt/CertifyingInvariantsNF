...
import IdealArithmetic.Examples.NF3_1_25160_1.PrimesBelowCert3_1_25160_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 29*X - 25`.
...
def SturmRC : SturmBuilderOfList [[-25, 29, -1, 1], [29, -2, 3], [196, -172], [-100640]] [-25, 29, -1, 1] [29, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-25, 29, -1, 1], [29, -2, 3], [196, -172], [-100640]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (44.89000775019161 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √25160 ≤ 158.61905000000002 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √25160 ≤ 35.258678 := by
