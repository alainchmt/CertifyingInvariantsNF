...
import IdealArithmetic.Examples.NF3_1_195615_2.PrimesBelowCert3_1_195615_2
...
/- Number field `K(α)` with α root of polynomial `X^3 - 27*X - 429`.
...
def SturmRC : SturmBuilderOfList [[-429, -27, 0, 1], [-27, 0, 3], [3861, 162], [-4890375]] [-429, -27, 0, 1] [-27, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-429, -27, 0, 1], [-27, 0, 3], [3861, 162], [-4890375]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (125.15072935603231 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √195615 ≤ 442.28385599999996 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √195615 ≤ 98.295301 := by
