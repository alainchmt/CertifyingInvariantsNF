...
import IdealArithmetic.Examples.NF5_1_250000_1.PrimesBelowCert5_1_250000_1
...
/- Number field `K(α)` with α root of polynomial `X^5 - 5*X^3 + 10*X - 4`.
...
def SturmRC : SturmBuilderOfList [[-4, 10, 0, -5, 0, 1], [10, 0, -15, 0, 5], [100, -200, 0, 50], [-1000, 1000, -500], [10000, 10000], [1000000]] [-4, 10, 0, -5, 0, 1] [10, 0, -15, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 2500, 250000, 100000000]
  f := [1, 25, 2500, 250000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [0, 250], [-50000, -25000], [15000000, -5000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-4, 10, 0, -5, 0, 1], [10, 0, -15, 0, 5], [100, -200, 0, 50], [-1000, 1000, -500], [10000, 10000], [1000000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (31.135867614926166 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √250000 ≤ 500.00001 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √250000 ≤ 19.21 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 32} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB32

def 𝔭' : Fin 9 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB32.β i ∈ List.ofFn PB32.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB32.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB32) (hc ▸ this) ))
