...
import IdealArithmetic.Examples.NF3_1_189283_1.PrimesBelowCert3_1_189283_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 60*X - 304`.
...
def SturmRC : SturmBuilderOfList [[-304, 60, -1, 1], [60, -2, 3], [2676, -358], [-3028528]] [-304, 60, -1, 1] [60, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-304, 60, -1, 1], [60, -2, 3], [2676, -358], [-3028528]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (123.10868402059812 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √189283 ≤ 435.066672 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √189283 ≤ 96.691483 := by
