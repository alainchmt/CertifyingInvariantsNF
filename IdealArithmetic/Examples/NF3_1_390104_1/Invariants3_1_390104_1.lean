...
import IdealArithmetic.Examples.NF3_1_390104_1.PrimesBelowCert3_1_390104_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 70*X - 306`.
...
def SturmRC : SturmBuilderOfList [[-306, 70, -1, 1], [70, -2, 3], [2684, -418], [-3510936]] [-306, 70, -1, 1] [70, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-306, 70, -1, 1], [70, -2, 3], [2684, -418], [-3510936]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (176.7308560326903 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √390104 ≤ 624.583071 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √390104 ≤ 138.806238 := by
