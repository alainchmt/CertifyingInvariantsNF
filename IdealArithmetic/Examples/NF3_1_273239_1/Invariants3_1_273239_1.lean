...
import IdealArithmetic.Examples.NF3_1_273239_1.PrimesBelowCert3_1_273239_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 14*X - 1111`.
...
def SturmRC : SturmBuilderOfList [[-1111, 14, -1, 1], [14, -2, 3], [9985, -82], [-33061919]] [-1111, 14, -1, 1] [14, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-1111, 14, -1, 1], [14, -2, 3], [9985, -82], [-33061919]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -33061919 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -273239 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (147.91026375087787 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √273239 ≤ 522.72269 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √273239 ≤ 116.170598 := by
