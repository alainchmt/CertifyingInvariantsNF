...
import IdealArithmetic.Examples.NF3_1_117175_1.PrimesBelowCert3_1_117175_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 25*X - 45`.
...
def SturmRC : SturmBuilderOfList [[-45, 25, 0, 1], [25, 0, 3], [405, -150], [-117175]] [-45, 25, 0, 1] [25, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-45, 25, 0, 1], [25, 0, 3], [405, -150], [-117175]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (96.86344792492378 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √117175 ≤ 342.30835099999996 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √117175 ≤ 76.078522 := by
