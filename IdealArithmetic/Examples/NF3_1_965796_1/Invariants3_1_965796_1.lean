...
import IdealArithmetic.Examples.NF3_1_965796_1.PrimesBelowCert3_1_965796_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 276*X - 1716`.
...
def SturmRC : SturmBuilderOfList [[-1716, -276, -1, 1], [-276, -2, 3], [15720, 1658], [-3863184]] [-1716, -276, -1, 1] [-276, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-1716, -276, -1, 1], [-276, -2, 3], [15720, 1658], [-3863184]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (278.0711445354664 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √965796 ≤ 982.7492149999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √965796 ≤ 218.398714 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 279} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB279

def 𝔭' : Fin 64 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB279.β i ∈ List.ofFn PB279.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB279.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB279) (hc ▸ this) ))
