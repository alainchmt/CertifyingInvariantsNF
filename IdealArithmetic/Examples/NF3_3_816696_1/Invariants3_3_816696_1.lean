...
import IdealArithmetic.Examples.NF3_3_816696_1.PrimesBelowCert3_3_816696_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 129*X - 214`.
...
def SturmRC : SturmBuilderOfList [[-214, -129, 0, 1], [-129, 0, 3], [1926, 774], [7350264]] [-214, -129, 0, 1] [-129, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-214, -129, 0, 1], [-129, 0, 3], [1926, 774], [7350264]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (200.83496524751763 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √816696 ≤ 903.712354 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √816696 ≤ 200.834968 := by
