...
import IdealArithmetic.Examples.NF8_0_12960000_2.PrimesBelowCert8_0_12960000_2
...
/- Number field `K(α)` with α root of polynomial `X^8 - X^6 + 2*X^4 + X^2 + 1`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `(1, a, a^2, a^3, a^4, a^5, 1/2*a^6 - 1/2, 1/2*a^7 - 1/2*a)` -/
...
def SturmRC : SturmBuilderOfList [[1, 0, 1, 0, 2, 0, -1, 0, 1], [0, 2, 0, 8, 0, -6, 0, 8], [-64, 0, -48, 0, -64, 0, 16], [0, -136, 0, -128, 0, -104], [2704, 0, 2912, 0, 3536], [0, 65280, 0, 48960], [-518400, 0, 345600], [0, -6912000], [207360000]] [1, 0, 1, 0, 2, 0, -1, 0, 1] [0, 2, 0, 8, 0, -6, 0, 8] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [64, 256, 10816, 12503296, 2397081600, 119439360000, 47775744000000]
  f := [1, 64, 256, 10816, 12503296, 2397081600, 119439360000]
  epos := by decide
  fpos := by decide
  Q := [[0, 8], [0, 128], [0, -1664], [0, -367744], [0, 173122560], [0, 16920576000], [0, -2388787200000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[1, 0, 1, 0, 2, 0, -1, 0, 1], [0, 2, 0, 8, 0, -6, 0, 8], [-64, 0, -48, 0, -64, 0, 16], [0, -136, 0, -128, 0, -104], [2704, 0, 2912, 0, 3536], [0, 65280, 0, 48960], [-518400, 0, 345600], [0, -6912000], [207360000]]
  SB := SturmRC
  k := 0
  r := 4
  hr := by decide
...
lemma T_discr : T.discr = 207360000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 12960000 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (22.74754663439851 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √12960000 ≤ 3600.00001 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.00240325927734375 * √12960000 ≤ 8.661733 := by
...
def BM : (i : Fin 1) → Fin (e i) → (Fin 1 → ℕ) := by 
  rintro ⟨i, hi⟩ 
  interval_cases i 
  exact ![![0], ![0], ![0], ![0], ![0], ![0]]

lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 23} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB23

def 𝔭' : Fin 6 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB23.β i ∈ List.ofFn PB23.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB23.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB23) (hc ▸ this) ))
...
lemma 𝔭'_apply : ∀ (i : Fin 6), ↑(𝔭' i) = (Fin.addCasesIter e 𝔭) i := by
  intro i
  rw [← cert_eq_𝔭]
  rfl
...
lemma relations_proof0 (i : Fin 6) : IsInClass O J (𝔭 0 i) (BM 0 i) := by
  show IsInClass O J (𝔭 0 i) (![![0], ![0], ![0], ![0], ![0], ![0]] i)
  unfold IsInClass
  fin_cases i
