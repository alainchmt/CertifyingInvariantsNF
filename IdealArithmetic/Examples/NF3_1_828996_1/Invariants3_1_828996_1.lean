...
import IdealArithmetic.Examples.NF3_1_828996_1.PrimesBelowCert3_1_828996_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 121*X + 879`.
...
def SturmRC : SturmBuilderOfList [[879, 121, -1, 1], [121, -2, 3], [-8032, -724], [-29843856]] [879, 121, -1, 1] [121, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[879, 121, -1, 1], [121, -2, 3], [-8032, -724], [-29843856]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -29843856 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -828996 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (257.6265870815895 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √828996 ≤ 910.492185 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √828996 ≤ 202.341597 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 258} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB258

def 𝔭' : Fin 52 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB258.β i ∈ List.ofFn PB258.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB258.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB258) (hc ▸ this) ))
