...
import IdealArithmetic.Examples.NF3_1_837620_1.PrimesBelowCert3_1_837620_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 37*X + 1747`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/10*a^2 - 2/5*a - 1/10]` -/
...
def SturmRC : SturmBuilderOfList [[1747, 37, -1, 1], [37, -2, 3], [-15760, -220], [-83762000]] [1747, 37, -1, 1] [37, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[1747, 37, -1, 1], [37, -2, 3], [-15760, -220], [-83762000]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -83762000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -837620 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (258.9631057675553 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √837620 ≤ 915.215832 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √837620 ≤ 203.391296 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 259} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB259

def 𝔭' : Fin 67 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB259.β i ∈ List.ofFn PB259.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB259.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB259) (hc ▸ this) ))
