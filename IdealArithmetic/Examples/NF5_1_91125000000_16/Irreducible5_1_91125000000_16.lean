...
def P13PI1 : CertificateIrreducibleZModOfList' 13 4 2 3 [2, 11, 7, 1, 1] where
 m := 1
 P := ![2]
 exp := ![2]
 hneq := by decide
...
def P23PI0 : CertificateIrreducibleZModOfList' 23 2 2 4 [10, 19, 1] where
 m := 1
 P := ![2]
 exp := ![1]
 hneq := by decide
...
def P23PI1 : CertificateIrreducibleZModOfList' 23 3 2 4 [20, 17, 4, 1] where
 m := 1
 P := ![3]
 exp := ![1]
 hneq := by decide
...
noncomputable def C : IrreducibleCertificateIntPolynomial T l where
 ...
 hirr := by
  ...
  · exact irreducible_ofList_of_linear (R := ZMod 13) _ (by decide) rfl
