...
import IdealArithmetic.Examples.NF3_3_181192_1.PrimesBelowCert3_3_181192_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 104*X - 212`.
...
def SturmRC : SturmBuilderOfList [[-212, -104, -1, 1], [-104, -2, 3], [2012, 626], [2899072]] [-212, -104, -1, 1] [-104, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-212, -104, -1, 1], [-104, -2, 3], [2012, 626], [2899072]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (94.60256358942681 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √181192 ≤ 425.666546 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √181192 ≤ 94.602566 := by
