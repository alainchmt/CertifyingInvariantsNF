...
import IdealArithmetic.Examples.NF5_1_1012500_1.PrimesBelowCert5_1_1012500_1
...
/- Number field `K(α)` with α root of polynomial `X^5 - 5*X^3 - 12`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, a^2, 1/2*a^3 - 1/2*a^2, 1/2*a^4 - 1/2*a^2]` -/
...
def SturmRC : SturmBuilderOfList [[-12, 0, 0, -5, 0, 1], [0, 0, -15, 0, 5], [300, 0, 0, 50], [0, 3000, 1500], [-270000, -180000], [16200000]] [-12, 0, 0, -5, 0, 1] [0, 0, -15, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 2500, 2250000, 32400000000]
  f := [1, 25, 2500, 2250000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [0, 250], [-150000, 75000], [-135000000, -270000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-12, 0, 0, -5, 0, 1], [0, 0, -15, 0, 5], [300, 0, 0, 50], [0, 3000, 1500], [-270000, -180000], [16200000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 16200000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 1012500 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (62.64960026107074 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √1012500 ≤ 1006.2306 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √1012500 ≤ 38.649255 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 63} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB63

def 𝔭' : Fin 16 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB63.β i ∈ List.ofFn PB63.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB63.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB63) (hc ▸ this) ))
