...
import IdealArithmetic.Examples.NF3_1_463091_1.PrimesBelowCert3_1_463091_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 4*X - 131`.
...
def SturmRC : SturmBuilderOfList [[-131, -4, 0, 1], [-4, 0, 3], [1179, 24], [-463091]] [-131, -4, 0, 1] [-4, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-131, -4, 0, 1], [-4, 0, 3], [1179, 24], [-463091]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (192.5543482108519 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √463091 ≤ 680.5079089999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √463091 ≤ 151.23398 := by
