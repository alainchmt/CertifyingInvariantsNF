...
import IdealArithmetic.Examples.NF3_1_31496_1.PrimesBelowCert3_1_31496_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 26*X - 98`.
...
def SturmRC : SturmBuilderOfList [[-98, 26, -1, 1], [26, -2, 3], [856, -154], [-283464]] [-98, 26, -1, 1] [26, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-98, 26, -1, 1], [26, -2, 3], [856, -154], [-283464]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (50.224056366613006 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √31496 ≤ 177.471134 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √31496 ≤ 39.44803 := by
