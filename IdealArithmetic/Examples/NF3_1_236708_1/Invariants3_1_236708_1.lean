...
import IdealArithmetic.Examples.NF3_1_236708_1.PrimesBelowCert3_1_236708_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 20*X - 94`.
...
def SturmRC : SturmBuilderOfList [[-94, 20, -1, 1], [20, -2, 3], [826, -118], [-236708]] [-94, 20, -1, 1] [20, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-94, 20, -1, 1], [20, -2, 3], [826, -118], [-236708]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (137.66882962210164 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √236708 ≤ 486.52647399999995 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √236708 ≤ 108.126994 := by
