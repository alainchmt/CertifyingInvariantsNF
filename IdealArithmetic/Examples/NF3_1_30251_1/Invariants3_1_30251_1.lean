import IdealArithmetic.Examples.NF3_1_30251_1.ClassGroupSaturated3_1_30251_1_3
import IdealArithmetic.Examples.NF3_1_30251_1.RelationIdeals3_1_30251_1F0
import IdealArithmetic.Examples.NF3_1_30251_1.RelationIdeals3_1_30251_1F1
import IdealArithmetic.Examples.NF3_1_30251_1.PrimesBelowCert3_1_30251_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 9*X - 170`.
...
def SturmRC : SturmBuilderOfList [[-170, 9, -1, 1], [9, -2, 3], [1521, -52], [-756275]] [-170, 9, -1, 1] [9, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-170, 9, -1, 1], [9, -2, 3], [1521, -52], [-756275]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (49.221598563517304 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √30251 ≤ 173.928156 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √30251 ≤ 38.660701 := by
...
def BM : (i : Fin 2) → Fin (e i) → (Fin 1 → ℕ) := by 
  rintro ⟨i, hi⟩ 
  interval_cases i 
  exact ![![2], ![1], ![0], ![0], ![0], ![0], ![1], ![2], ![1], ![1], ![2], ![2], ![2], ![1]]
  exact ![![1], ![1], ![2], ![2], ![2]]

lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 50} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB50

def 𝔭' : Fin 19 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB50.β i ∈ List.ofFn PB50.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB50.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB50) (hc ▸ this) ))
...
lemma 𝔭'_apply : ∀ (i : Fin 19), ↑(𝔭' i) = (Fin.addCasesIter e 𝔭) i := by
  intro i
  rw [← cert_eq_𝔭]
  rfl
...
lemma relations_proof0 (i : Fin 14) : IsInClass O J (𝔭 0 i) (BM 0 i) := by
  show IsInClass O J (𝔭 0 i) (![![2], ![1], ![0], ![0], ![0], ![0], ![1], ![2], ![1], ![1], ![2], ![2], ![2], ![1]] i)
  unfold IsInClass
...
lemma relations_proof1 (i : Fin 5) : IsInClass O J (𝔭 1 i) (BM 1 i) := by
  show IsInClass O J (𝔭 1 i) (![![1], ![1], ![2], ![2], ![2]] i)
  unfold IsInClass
  simp only [Fin.isValue, Fin.prod_univ_castSucc, Finset.univ_eq_empty, Finset.prod_empty, Fin.succ_zero_eq_one, one_mul]
  fin_cases i
