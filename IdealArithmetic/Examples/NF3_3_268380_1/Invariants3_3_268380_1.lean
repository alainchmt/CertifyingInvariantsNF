...
import IdealArithmetic.Examples.NF3_3_268380_1.PrimesBelowCert3_3_268380_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 87*X - 296`.
...
def SturmRC : SturmBuilderOfList [[-296, -87, 0, 1], [-87, 0, 3], [2664, 522], [268380]] [-296, -87, 0, 1] [-87, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-296, -87, 0, 1], [-87, 0, 3], [2664, 522], [268380]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (115.13312249645305 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √268380 ≤ 518.0540609999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √268380 ≤ 115.133125 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 116} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB116

def 𝔭' : Fin 32 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB116.β i ∈ List.ofFn PB116.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB116.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB116) (hc ▸ this) ))
