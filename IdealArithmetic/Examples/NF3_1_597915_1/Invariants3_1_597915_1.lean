...
import IdealArithmetic.Examples.NF3_1_597915_1.PrimesBelowCert3_1_597915_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 147*X - 1374`.
...
def SturmRC : SturmBuilderOfList [[-1374, -147, 0, 1], [-147, 0, 3], [12366, 882], [-38266560]] [-1374, -147, 0, 1] [-147, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-1374, -147, 0, 1], [-147, 0, 3], [12366, 882], [-38266560]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -38266560 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -597915 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (218.79489219064197 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √597915 ≤ 773.249646 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √597915 ≤ 171.843255 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 219} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB219

def 𝔭' : Fin 48 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB219.β i ∈ List.ofFn PB219.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB219.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB219) (hc ▸ this) ))
