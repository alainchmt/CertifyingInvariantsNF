import IdealArithmetic.Examples.NF3_1_32780_1.ClassGroupSaturated3_1_32780_1_2
import IdealArithmetic.Examples.NF3_1_32780_1.RelationIdeals3_1_32780_1F0
import IdealArithmetic.Examples.NF3_1_32780_1.RelationIdeals3_1_32780_1F1
import IdealArithmetic.Examples.NF3_1_32780_1.PrimesBelowCert3_1_32780_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 8*X - 174`.
Class group structure `[4]`, generated by class of ideals `[(3, a - 1)]`. -/
...
def SturmRC : SturmBuilderOfList [[-174, 8, 0, 1], [8, 0, 3], [1566, -48], [-819500]] [-174, 8, 0, 1] [8, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-174, 8, 0, 1], [8, 0, 3], [1566, -48], [-819500]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
def NPSU2 : pSaturatedClassGroupCertificateDvdT 2 ![J0] ![4] where 
 ...
 hDl := by decide 
 NInv := ![![1, 1, 0], ![1, 1, 1], ![0, 1, 1]]
 hN := by decide 

lemma T_discr : T.discr = -819500 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -32780 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (51.237372311223965 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √32780 ≤ 181.052489 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √32780 ≤ 40.243886 := by
...
def BM : (i : Fin 2) → Fin (e i) → (Fin 1 → ℕ) := by 
  rintro ⟨i, hi⟩ 
  interval_cases i 
  exact ![![0], ![1], ![2], ![1], ![1], ![2], ![2], ![1], ![3], ![1], ![3]]
  exact ![![1], ![1], ![0], ![3], ![3], ![1]]

lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 52} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB52

def 𝔭' : Fin 17 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB52.β i ∈ List.ofFn PB52.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB52.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB52) (hc ▸ this) ))
...
lemma 𝔭'_apply : ∀ (i : Fin 17), ↑(𝔭' i) = (Fin.addCasesIter e 𝔭) i := by
  intro i
  rw [← cert_eq_𝔭]
  rfl
...
lemma relations_proof0 (i : Fin 11) : IsInClass O J (𝔭 0 i) (BM 0 i) := by
  show IsInClass O J (𝔭 0 i) (![![0], ![1], ![2], ![1], ![1], ![2], ![2], ![1], ![3], ![1], ![3]] i)
  unfold IsInClass
...
lemma relations_proof1 (i : Fin 6) : IsInClass O J (𝔭 1 i) (BM 1 i) := by
  show IsInClass O J (𝔭 1 i) (![![1], ![1], ![0], ![3], ![3], ![1]] i)
  unfold IsInClass
  simp only [Fin.isValue, Fin.prod_univ_castSucc, Finset.univ_eq_empty, Finset.prod_empty, Fin.succ_zero_eq_one, one_mul]
  fin_cases i
