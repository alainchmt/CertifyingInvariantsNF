...
import IdealArithmetic.Examples.NF3_3_376300_1.PrimesBelowCert3_3_376300_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 168*X - 98`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/7*a^2 - 1/7*a]` -/
...
def SturmRC : SturmBuilderOfList [[-98, -168, -1, 1], [-168, -2, 3], [1050, 1010], [18438700]] [-98, -168, -1, 1] [-168, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-98, -168, -1, 1], [-168, -2, 3], [1050, 1010], [18438700]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 18438700 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 376300 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (136.32843620502223 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √376300 ≤ 613.432973 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √376300 ≤ 136.328438 := by
