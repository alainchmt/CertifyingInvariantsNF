...
import IdealArithmetic.Examples.NF3_3_97825_1.PrimesBelowCert3_3_97825_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 55*X - 145`.
...
def SturmRC : SturmBuilderOfList [[-145, -55, 0, 1], [-55, 0, 3], [1305, 330], [97825]] [-145, -55, 0, 1] [-55, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-145, -55, 0, 1], [-55, 0, 3], [1305, 330], [97825]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (69.51441854681518 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √97825 ≤ 312.76989299999997 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √97825 ≤ 69.514421 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 70} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB70

def 𝔭' : Fin 26 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB70.β i ∈ List.ofFn PB70.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB70.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB70) (hc ▸ this) ))
