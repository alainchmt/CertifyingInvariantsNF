...
import IdealArithmetic.Examples.NF3_3_874321_1.PrimesBelowCert3_3_874321_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 162*X - 531`.
...
def SturmRC : SturmBuilderOfList [[-531, -162, -1, 1], [-162, -2, 3], [4941, 974], [7868889]] [-531, -162, -1, 1] [-162, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-531, -162, -1, 1], [-162, -2, 3], [4941, 974], [7868889]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (207.79918566425044 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √874321 ≤ 935.051345 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √874321 ≤ 207.799188 := by
