...
import IdealArithmetic.Examples.NF3_3_902356_1.PrimesBelowCert3_3_902356_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 178*X - 2`.
...
def SturmRC : SturmBuilderOfList [[-2, -178, 0, 1], [-178, 0, 3], [18, 1068], [22558900]] [-2, -178, 0, 1] [-178, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-2, -178, 0, 1], [-178, 0, 3], [18, 1068], [22558900]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 22558900 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 902356 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (211.10426833397628 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √902356 ≤ 949.924218 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √902356 ≤ 211.104271 := by
