import IdealArithmetic.Examples.NF3_1_780300_1.Invariants3_1_780300_1

noncomputable section

open Polynomial NumberField

/- Number field `K(α)` with `α` root of the polynomial `X^3 - 850`. -/

lemma T_def' : K = AdjoinRoot (map (algebraMap ℤ ℚ) (X^3 - 850)) := rfl

lemma T_irreducible' : Irreducible (X^3 - 850 : ℤ[X]) := irreducible_T

theorem O_ringOfIntegers : O = RingOfIntegers K := O_ringOfIntegers'

theorem K_discr' : discr K = -780300 := K_discr
