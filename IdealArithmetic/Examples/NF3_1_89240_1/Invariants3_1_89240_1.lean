...
import IdealArithmetic.Examples.NF3_1_89240_1.PrimesBelowCert3_1_89240_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 460`.
...
def SturmRC : SturmBuilderOfList [[460, 0, -1, 1], [0, -2, 3], [-4140, 2], [-5711360]] [460, 0, -1, 1] [0, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[460, 0, -1, 1], [0, -2, 3], [-4140, 2], [-5711360]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (84.53348314262584 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √89240 ≤ 298.73065799999995 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √89240 ≤ 66.394591 := by
