...
import IdealArithmetic.Examples.NF3_1_304556_1.PrimesBelowCert3_1_304556_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 49*X - 329`.
...
def SturmRC : SturmBuilderOfList [[-329, -49, -1, 1], [-49, -2, 3], [3010, 296], [-2741004]] [-329, -49, -1, 1] [-49, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-329, -49, -1, 1], [-49, -2, 3], [3010, 296], [-2741004]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (156.1561155671722 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √304556 ≤ 551.8659359999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √304556 ≤ 122.646875 := by
