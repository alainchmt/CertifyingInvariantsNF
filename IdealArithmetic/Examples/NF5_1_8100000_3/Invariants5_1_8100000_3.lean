...
import IdealArithmetic.Examples.NF5_1_8100000_3.PrimesBelowCert5_1_8100000_3
...
/- Number field `K(α)` with α root of polynomial `X^5 - 10*X^2 + 15*X + 6`.
...
def SturmRC : SturmBuilderOfList [[6, 15, -10, 0, 0, 1], [15, -20, 0, 0, 5], [-150, -300, 150], [-1080000, -1080000], [-518400000]] [6, 15, -10, 0, 0, 1] [15, -20, 0, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 3375000, 1166400000000]
  f := [1, 125, 675000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [562500, 225000, 112500], [486000000, -162000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[6, 15, -10, 0, 0, 1], [15, -20, 0, 0, 5], [-150, -300, 150], [-1080000, -1080000], [-518400000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 518400000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 8100000 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (177.18154446167102 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √8100000 ≤ 2846.0499040000004 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √8100000 ≤ 109.298316 := by
