...
import IdealArithmetic.Examples.NF3_1_795800_1.PrimesBelowCert3_1_795800_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 18*X + 522`.
...
def SturmRC : SturmBuilderOfList [[522, -18, -1, 1], [-18, -2, 3], [-4680, 110], [-7162200]] [522, -18, -1, 1] [-18, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[522, -18, -1, 1], [-18, -2, 3], [-4680, 110], [-7162200]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (252.41594063572546 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √795800 ≤ 892.07624 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √795800 ≤ 198.249164 := by
