...
import IdealArithmetic.Examples.NF3_1_432012_1.PrimesBelowCert3_1_432012_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 37*X + 81`.
...
def SturmRC : SturmBuilderOfList [[81, 37, -1, 1], [37, -2, 3], [-766, -220], [-432012]] [81, 37, -1, 1] [37, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[81, 37, -1, 1], [37, -2, 3], [-766, -220], [-432012]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (185.98112148084806 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √432012 ≤ 657.276208 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √432012 ≤ 146.07138 := by
