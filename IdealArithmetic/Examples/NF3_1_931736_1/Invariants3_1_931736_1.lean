...
import IdealArithmetic.Examples.NF3_1_931736_1.PrimesBelowCert3_1_931736_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 96*X - 488`.
...
def SturmRC : SturmBuilderOfList [[-488, -96, -1, 1], [-96, -2, 3], [4488, 578], [-3726944]] [-488, -96, -1, 1] [-96, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-488, -96, -1, 1], [-96, -2, 3], [4488, 578], [-3726944]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (273.1240502323395 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √931736 ≤ 965.264741 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √931736 ≤ 214.513276 := by
...
def 𝔭' : Fin 72 → nonZeroDivisors (Ideal O) := by
  intro i
...
lemma 𝔭'_apply : ∀ (i : Fin 72), ↑(𝔭' i) = (Fin.addCasesIter e 𝔭) i := by
  intro i
  rw [← cert_eq_𝔭]
  rfl
