...
import IdealArithmetic.Examples.NF5_5_390625_1.PrimesBelowCert5_5_390625_1
...
/- Number field `K(α)` with α root of polynomial `X^5 - 10*X^3 - 5*X^2 + 10*X - 1`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, a^2, a^3, 1/7*a^4 - 3/7*a^3 - 1/7*a^2 - 2/7*a + 2/7]` -/
...
def SturmRC : SturmBuilderOfList [[-1, 10, -5, -10, 0, 1], [10, -10, -30, 0, 5], [25, -200, 75, 100], [-4375, 7500, 6875], [-15625, 468750], [19140625]] [-1, 10, -5, -10, 0, 1] [10, -10, -30, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 10000, 47265625, 219726562500]
  f := [1, 25, 10000, 47265625]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [-375, 500], [-234375, 687500], [3623046875, 3222656250]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-1, 10, -5, -10, 0, 1], [10, -10, -30, 0, 5], [25, -200, 75, 100], [-4375, 7500, 6875], [-15625, 468750], [19140625]]
  SB := SturmRC
  k := 5
  r := 5
  hr := by decide
...
lemma T_discr : T.discr = 19140625 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 390625 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (24.01 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √390625 ≤ 625.00001 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √390625 ≤ 24.01 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 25} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB25

def 𝔭' : Fin 6 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB25.β i ∈ List.ofFn PB25.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB25.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB25) (hc ▸ this) ))
