...
import IdealArithmetic.Examples.NF3_3_653844_1.PrimesBelowCert3_3_653844_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 138*X - 414`.
...
def SturmRC : SturmBuilderOfList [[-414, -138, 0, 1], [-138, 0, 3], [3726, 828], [5884596]] [-414, -138, 0, 1] [-138, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-414, -138, 0, 1], [-138, 0, 3], [3726, 828], [5884596]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (179.700268497191 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √653844 ≤ 808.606218 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √653844 ≤ 179.700271 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 180} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB180

def 𝔭' : Fin 42 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB180.β i ∈ List.ofFn PB180.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB180.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB180) (hc ▸ this) ))
