...
import IdealArithmetic.Examples.NF5_3_42187500_1.PrimesBelowCert5_3_42187500_1
...
/- Number field `K(α)` with α root of polynomial `X^5 - 10*X^3 - 20*X^2 - 15*X + 236`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/2*a^2 - 1/2*a, 1/4*a^3 - 1/4*a, 1/16*a^4 + 1/16*a^3 - 1/16*a^2 - 5/16*a + 1/4]` -/
...
def SturmRC : SturmBuilderOfList [[236, -15, -20, -10, 0, 1], [-15, -40, -30, 0, 5], [-5900, 300, 300, 100], [360000, -120000], [-691200000000]] [236, -15, -20, -10, 0, 1] [-15, -40, -30, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 10000, 1728000000000000]
  f := [1, 25, 1000000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [-1500, 500], [-30240000000000, -8640000000000, -1440000000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[236, -15, -20, -10, 0, 1], [-15, -40, -30, 0, 5], [-5900, 300, 300, 100], [360000, -120000], [-691200000000]]
  SB := SturmRC
  k := 3
  r := 4
  hr := by decide
...
lemma T_discr : T.discr = -691200000000 :=  by
...
theorem K_minowski : minkowskiBoundFB K ≤ (317.5754437629522 : ℝ) := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 318} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB318

def 𝔭' : Fin 58 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB318.β i ∈ List.ofFn PB318.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB318.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB318) (hc ▸ this) ))
