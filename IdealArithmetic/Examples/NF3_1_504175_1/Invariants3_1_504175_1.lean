...
import IdealArithmetic.Examples.NF3_1_504175_1.PrimesBelowCert3_1_504175_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 125*X - 555`.
...
def SturmRC : SturmBuilderOfList [[-555, -125, 0, 1], [-125, 0, 3], [4995, 750], [-504175]] [-555, -125, 0, 1] [-125, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-555, -125, 0, 1], [-125, 0, 3], [4995, 750], [-504175]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (200.91384951740991 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √504175 ≤ 710.052825 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √504175 ≤ 157.799517 := by
