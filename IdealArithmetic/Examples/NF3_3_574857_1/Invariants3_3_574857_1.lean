...
import IdealArithmetic.Examples.NF3_3_574857_1.PrimesBelowCert3_3_574857_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 75*X - 203`.
...
def SturmRC : SturmBuilderOfList [[-203, -75, 0, 1], [-75, 0, 3], [1827, 450], [574857]] [-203, -75, 0, 1] [-75, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-203, -75, 0, 1], [-75, 0, 3], [1827, 450], [574857]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (168.49738825205878 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √574857 ≤ 758.193257 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √574857 ≤ 168.49739 := by
