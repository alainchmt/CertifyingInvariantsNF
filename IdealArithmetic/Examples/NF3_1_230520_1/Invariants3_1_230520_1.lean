...
import IdealArithmetic.Examples.NF3_1_230520_1.PrimesBelowCert3_1_230520_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 55*X + 595`.
...
def SturmRC : SturmBuilderOfList [[595, -55, -1, 1], [-55, -2, 3], [-5300, 332], [-8298720]] [595, -55, -1, 1] [-55, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[595, -55, -1, 1], [-55, -2, 3], [-5300, 332], [-8298720]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (135.85758126623668 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √230520 ≤ 480.12499399999996 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √230520 ≤ 106.704443 := by
