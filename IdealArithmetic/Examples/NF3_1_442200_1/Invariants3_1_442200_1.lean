...
import IdealArithmetic.Examples.NF3_1_442200_1.PrimesBelowCert3_1_442200_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 17*X - 773`.
...
def SturmRC : SturmBuilderOfList [[-773, 17, -1, 1], [17, -2, 3], [6940, -100], [-15919200]] [-773, 17, -1, 1] [17, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-773, 17, -1, 1], [17, -2, 3], [6940, -100], [-15919200]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -15919200 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -442200 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (188.16119196366145 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √442200 ≤ 664.981213 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √442200 ≤ 147.783603 := by
