...
import IdealArithmetic.Examples.NF3_1_470372_1.PrimesBelowCert3_1_470372_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 158*X - 1076`.
...
def SturmRC : SturmBuilderOfList [[-1076, 158, 0, 1], [158, 0, 3], [9684, -948], [-47037200]] [-1076, 158, 0, 1] [158, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-1076, 158, 0, 1], [158, 0, 3], [9684, -948], [-47037200]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -47037200 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -470372 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (194.06209491146154 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √470372 ≤ 685.836725 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √470372 ≤ 152.418161 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 195} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB195

def 𝔭' : Fin 50 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB195.β i ∈ List.ofFn PB195.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB195.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB195) (hc ▸ this) ))
