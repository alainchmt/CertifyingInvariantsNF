...
import IdealArithmetic.Examples.NF3_1_528447_1.PrimesBelowCert3_1_528447_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 105*X - 936`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/6*a^2 - 1/2*a]` -/
...
def SturmRC : SturmBuilderOfList [[-936, -105, 0, 1], [-105, 0, 3], [8424, 630], [-19024092]] [-936, -105, 0, 1] [-105, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-936, -105, 0, 1], [-105, 0, 3], [8424, 630], [-19024092]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -19024092 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -528447 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (205.69296454013573 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √528447 ≤ 726.943612 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √528447 ≤ 161.553025 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 206} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB206

def 𝔭' : Fin 49 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB206.β i ∈ List.ofFn PB206.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB206.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB206) (hc ▸ this) ))
