...
import IdealArithmetic.Examples.NF3_1_589784_1.PrimesBelowCert3_1_589784_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 110*X - 2`.
...
def SturmRC : SturmBuilderOfList [[-2, 110, -1, 1], [110, -2, 3], [-92, -658], [-5308056]] [-2, 110, -1, 1] [110, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-2, 110, -1, 1], [110, -2, 3], [-92, -658], [-5308056]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (217.30218055880434 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √589784 ≤ 767.973968 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √589784 ≤ 170.670882 := by
