...
import IdealArithmetic.Examples.NF5_1_24300000_2.PrimesBelowCert5_1_24300000_2
...
/- Number field `K(α)` with α root of polynomial `X^5 - 10*X^2 + 15*X - 42`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, a^2, 1/2*a^3 - 1/2*a, 1/12*a^4 + 1/12*a^3 - 5/12*a^2 - 1/4*a - 1/2]` -/
...
def SturmRC : SturmBuilderOfList [[-42, 15, -10, 0, 0, 1], [15, -20, 0, 0, 5], [1050, -300, 150], [-3240000, 3240000], [-13996800000]] [-42, 15, -10, 0, 0, 1] [15, -20, 0, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 3375000, 10497600000000]
  f := [1, 125, 675000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [-337500, 225000, 112500], [-486000000, 486000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-42, 15, -10, 0, 0, 1], [15, -20, 0, 0, 5], [1050, -300, 150], [-3240000, 3240000], [-13996800000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 13996800000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 24300000 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (306.88011666306255 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √24300000 ≤ 4929.503028 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √24300000 ≤ 189.302916 := by
...
def BM : (i : Fin 7) → Fin (e i) → (Fin 1 → ℕ) := by 
  rintro ⟨i, hi⟩ 
  interval_cases i 
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 307} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB307

def 𝔭' : Fin 59 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB307.β i ∈ List.ofFn PB307.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB307.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB307) (hc ▸ this) ))
