...
import IdealArithmetic.Examples.NF3_1_249879_1.PrimesBelowCert3_1_249879_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 30*X - 192`.
...
def SturmRC : SturmBuilderOfList [[-192, 30, -1, 1], [30, -2, 3], [1698, -178], [-999516]] [-192, 30, -1, 1] [30, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-192, 30, -1, 1], [30, -2, 3], [1698, -178], [-999516]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (141.44682038592347 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √249879 ≤ 499.878995 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √249879 ≤ 111.094221 := by
