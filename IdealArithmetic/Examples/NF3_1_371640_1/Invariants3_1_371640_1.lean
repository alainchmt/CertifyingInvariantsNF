...
import IdealArithmetic.Examples.NF3_1_371640_1.PrimesBelowCert3_1_371640_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 125*X + 415`.
...
def SturmRC : SturmBuilderOfList [[415, 125, -1, 1], [125, -2, 3], [-3860, -748], [-13379040]] [415, 125, -1, 1] [125, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[415, 125, -1, 1], [125, -2, 3], [-3860, -748], [-13379040]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -13379040 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -371640 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (172.49797775553319 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √371640 ≤ 609.622844 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √371640 ≤ 135.481743 := by
