...
import IdealArithmetic.Examples.NF3_3_662216_1.PrimesBelowCert3_3_662216_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 115*X - 321`.
...
def SturmRC : SturmBuilderOfList [[-321, -115, -1, 1], [-115, -2, 3], [3004, 692], [2648864]] [-321, -115, -1, 1] [-115, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-321, -115, -1, 1], [-115, -2, 3], [3004, 692], [2648864]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (180.84701139799347 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √662216 ≤ 813.766561 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √662216 ≤ 180.847014 := by
