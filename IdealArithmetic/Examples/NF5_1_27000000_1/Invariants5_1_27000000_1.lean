...
import IdealArithmetic.Examples.NF5_1_27000000_1.PrimesBelowCert5_1_27000000_1
...
/- Number field `K(α)` with α root of polynomial `X^5 + 5*X^3 - 10*X^2 + 64`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/2*a^2 - 1/2*a, 1/4*a^3 - 1/4*a^2 - 1/2*a, 1/8*a^4 + 1/8*a^2 + 1/4*a]` -/
...
def SturmRC : SturmBuilderOfList [[64, 0, -10, 5, 0, 1], [0, -20, 15, 0, 5], [-1600, 0, 150, -50], [48000, 18000, -6000], [23040000, 5760000], [110592000000]] [64, 0, -10, 5, 0, 1] [0, -20, 15, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 2500, 36000000, 33177600000000]
  f := [1, 25, 2500, 36000000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [-750, -250], [0, 300000], [241920000000, -34560000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[64, 0, -10, 5, 0, 1], [0, -20, 15, 0, 5], [-1600, 0, 150, -50], [48000, 18000, -6000], [23040000, 5760000], [110592000000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 110592000000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 27000000 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (323.47950483228897 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √27000000 ≤ 5196.152433 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √27000000 ≤ 199.542253 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 324} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB324

def 𝔭' : Fin 61 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB324.β i ∈ List.ofFn PB324.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB324.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB324) (hc ▸ this) ))
