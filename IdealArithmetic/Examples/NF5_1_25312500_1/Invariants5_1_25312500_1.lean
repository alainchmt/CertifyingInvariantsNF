...
import IdealArithmetic.Examples.NF5_1_25312500_1.PrimesBelowCert5_1_25312500_1
...
/- Number field `K(α)` with α root of polynomial `X^5 - 30*X^2 - 135*X - 252`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, a^2, 1/3*a^3, 1/246*a^4 - 3/82*a^3 + 27/82*a^2 - 7/82*a + 9/41]` -/
...
theorem O_integral_closure : O = integralClosure ℤ K := by
  ...
  · haveI : Fact $ Nat.Prime p := fact_iff.2 hp
    refine piMaximal_of_root_in_order_of_satisfiesDedekindCriterion_int Adj T_monic hm ?_ hroot_mem
     (satisfiesDedekindAlmostAllLists_of_certificate T _ T_ofList [41, 2, 3] D p hp hc)
...
def SturmRC : SturmBuilderOfList [[-252, -135, -30, 0, 0, 1], [-135, -60, 0, 0, 5], [6300, 2700, 450], [1221075000, 218700000], [-13786301250000]] [-252, -135, -30, 0, 0, 1] [-135, -60, 0, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 91125000, 47829690000000000]
  f := [1, 125, 18225000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [22275000, -6075000, 1012500], [41006250000, 98415000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-252, -135, -30, 0, 0, 1], [-135, -60, 0, 0, 5], [6300, 2700, 450], [1221075000, 218700000], [-13786301250000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 13786301250000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 25312500 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 314} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB314

def 𝔭' : Fin 68 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB314.β i ∈ List.ofFn PB314.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB314.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB314) (hc ▸ this) ))
...
lemma relations_proof7 (i : Fin 3) : IsInClass O J (𝔭 7 i) (BM 7 i) := by
  show IsInClass O J (𝔭 7 i) (![![0], ![0], ![0]] i)
  unfold IsInClass
  fin_cases i
  · refine ideal_mem_principal_class''' (by decide) J rfl rfl
  · refine ideal_mem_principal_class''' (by decide) J rfl rfl
