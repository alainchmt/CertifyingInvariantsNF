...
import IdealArithmetic.Examples.NF3_1_737124_1.PrimesBelowCert3_1_737124_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 16*X - 656`.
...
def SturmRC : SturmBuilderOfList [[-656, -16, -1, 1], [-16, -2, 3], [5920, 98], [-11793984]] [-656, -16, -1, 1] [-16, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-656, -16, -1, 1], [-16, -2, 3], [5920, 98], [-11793984]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -11793984 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -737124 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (242.93257759037195 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √737124 ≤ 858.559268 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √737124 ≤ 190.800948 := by
