...
import IdealArithmetic.Examples.NF3_3_392472_1.PrimesBelowCert3_3_392472_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 75*X - 66`.
...
def SturmRC : SturmBuilderOfList [[-66, -75, 0, 1], [-75, 0, 3], [594, 450], [1569888]] [-66, -75, 0, 1] [-75, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-66, -75, 0, 1], [-75, 0, 3], [594, 450], [1569888]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (139.22685721683752 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √392472 ≤ 626.475867 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √392472 ≤ 139.226859 := by
