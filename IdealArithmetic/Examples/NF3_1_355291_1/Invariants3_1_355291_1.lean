...
import IdealArithmetic.Examples.NF3_1_355291_1.PrimesBelowCert3_1_355291_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 89*X + 896`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/7*a^2 + 3/7*a]` -/
...
def SturmRC : SturmBuilderOfList [[896, -89, -1, 1], [-89, -2, 3], [-7975, 536], [-17409259]] [896, -89, -1, 1] [-89, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[896, -89, -1, 1], [-89, -2, 3], [-7975, 536], [-17409259]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -17409259 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -355291 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (168.66130577388194 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √355291 ≤ 596.062926 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √355291 ≤ 132.468428 := by
