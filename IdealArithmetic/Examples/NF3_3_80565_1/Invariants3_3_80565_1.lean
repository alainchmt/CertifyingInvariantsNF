...
import IdealArithmetic.Examples.NF3_3_80565_1.PrimesBelowCert3_3_80565_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 61*X + 106`.
...
def SturmRC : SturmBuilderOfList [[106, -61, -1, 1], [-61, -2, 3], [-893, 368], [725085]] [106, -61, -1, 1] [-61, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[106, -61, -1, 1], [-61, -2, 3], [-893, 368], [725085]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (63.08549855941305 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √80565 ≤ 283.83975399999997 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √80565 ≤ 63.085501 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 64} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB64

def 𝔭' : Fin 20 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB64.β i ∈ List.ofFn PB64.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB64.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB64) (hc ▸ this) ))
