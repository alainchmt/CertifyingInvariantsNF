...
import IdealArithmetic.Examples.NF3_1_383732_1.PrimesBelowCert3_1_383732_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 44*X + 390`.
...
def SturmRC : SturmBuilderOfList [[390, -44, -1, 1], [-44, -2, 3], [-3466, 266], [-3453588]] [390, -44, -1, 1] [-44, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[390, -44, -1, 1], [-44, -2, 3], [-3466, 266], [-3453588]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (175.2816251166045 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √383732 ≤ 619.461066 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √383732 ≤ 137.668015 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 176} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB176

def 𝔭' : Fin 44 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB176.β i ∈ List.ofFn PB176.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB176.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB176) (hc ▸ this) ))
