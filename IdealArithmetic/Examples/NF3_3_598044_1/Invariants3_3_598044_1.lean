...
import IdealArithmetic.Examples.NF3_3_598044_1.PrimesBelowCert3_3_598044_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 84*X + 12`.
...
def SturmRC : SturmBuilderOfList [[12, -84, -1, 1], [-84, -2, 3], [-24, 506], [2392176]] [12, -84, -1, 1] [-84, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[12, -84, -1, 1], [-84, -2, 3], [-24, 506], [2392176]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (171.86178799487956 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √598044 ≤ 773.3330559999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √598044 ≤ 171.86179 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 172} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB172

def 𝔭' : Fin 43 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB172.β i ∈ List.ofFn PB172.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB172.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB172) (hc ▸ this) ))
