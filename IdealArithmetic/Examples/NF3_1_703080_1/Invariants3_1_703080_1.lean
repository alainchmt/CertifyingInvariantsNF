...
import IdealArithmetic.Examples.NF3_1_703080_1.PrimesBelowCert3_1_703080_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 78*X - 184`.
...
def SturmRC : SturmBuilderOfList [[-184, 78, 0, 1], [78, 0, 3], [1656, -468], [-2812320]] [-184, 78, 0, 1] [78, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-184, 78, 0, 1], [78, 0, 3], [1656, -468], [-2812320]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (237.25658888378453 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √703080 ≤ 838.498668 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √703080 ≤ 186.343037 := by
