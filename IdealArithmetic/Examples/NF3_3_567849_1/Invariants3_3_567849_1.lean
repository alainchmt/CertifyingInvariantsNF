...
import IdealArithmetic.Examples.NF3_3_567849_1.PrimesBelowCert3_3_567849_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 94*X + 232`.
...
def SturmRC : SturmBuilderOfList [[232, -94, -1, 1], [-94, -2, 3], [-1994, 566], [2271396]] [232, -94, -1, 1] [-94, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[232, -94, -1, 1], [-94, -2, 3], [-1994, 566], [2271396]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (167.46723611097227 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √567849 ≤ 753.5575719999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √567849 ≤ 167.467238 := by
