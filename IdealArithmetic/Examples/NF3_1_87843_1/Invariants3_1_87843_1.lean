...
import IdealArithmetic.Examples.NF3_1_87843_1.PrimesBelowCert3_1_87843_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 27*X - 30`.
...
def SturmRC : SturmBuilderOfList [[-30, 27, -1, 1], [27, -2, 3], [243, -160], [-87843]] [-30, 27, -1, 1] [27, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-30, 27, -1, 1], [27, -2, 3], [243, -160], [-87843]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (83.86929065930372 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √87843 ≤ 296.38320799999997 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √87843 ≤ 65.872935 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 84} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB84

def 𝔭' : Fin 24 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB84.β i ∈ List.ofFn PB84.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB84.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB84) (hc ▸ this) ))
...
lemma 𝔭'_apply : ∀ (i : Fin 24), ↑(𝔭' i) = (Fin.addCasesIter e 𝔭) i := by
  intro i
  rw [← cert_eq_𝔭]
  rfl
