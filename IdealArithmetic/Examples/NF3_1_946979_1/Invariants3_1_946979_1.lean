...
import IdealArithmetic.Examples.NF3_1_946979_1.PrimesBelowCert3_1_946979_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 44*X - 2057`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/11*a^2]` -/
...
theorem O_integral_closure : O = integralClosure ℤ K := by
  ...
  by_cases hc : p ∈ [11]
  · fin_cases hc
    exact @pMaximal_of_MaximalOrderCertificateLists K 11 _ IsAddTorsionFree.to_noZeroSMulDivisors_int _ O Om hm _ _ M11
...
def SturmRC : SturmBuilderOfList [[-2057, 44, 0, 1], [44, 0, 3], [18513, -264], [-114584459]] [-2057, 44, 0, 1] [44, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-2057, 44, 0, 1], [44, 0, 3], [18513, -264], [-114584459]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -114584459 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -946979 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (275.34903057785954 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √946979 ≤ 973.12847 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √946979 ≤ 216.260771 := by
...
def 𝔭' : Fin 75 → nonZeroDivisors (Ideal O) := by
  intro i
...
lemma 𝔭'_apply : ∀ (i : Fin 75), ↑(𝔭' i) = (Fin.addCasesIter e 𝔭) i := by
  intro i
  rw [← cert_eq_𝔭]
  rfl
