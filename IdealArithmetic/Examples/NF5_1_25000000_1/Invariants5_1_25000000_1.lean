...
import IdealArithmetic.Examples.NF5_1_25000000_1.PrimesBelowCert5_1_25000000_1
...
/- Number field `K(α)` with α root of polynomial `X^5 + 10*X^3 - 40*X^2 - 15*X + 88`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, a^2, 1/4*a^3 - 1/2*a^2 - 1/4*a, 1/28*a^4 + 3/28*a^3 + 5/28*a^2 + 3/28*a + 2/7]` -/
...
def SturmRC : SturmBuilderOfList [[88, -15, -40, 10, 0, 1], [-15, -80, 30, 0, 5], [-2200, 300, 600, -100], [270000, 40000, -90000], [432000000, -200000000], [313600000000]] [88, -15, -40, 10, 0, 1] [-15, -80, 30, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 10000, 8100000000, 40000000000000000]
  f := [1, 25, 10000, 8100000000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [-3000, -500], [-50000000, 9000000], [30880000000000, 18000000000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[88, -15, -40, 10, 0, 1], [-15, -80, 30, 0, 5], [-2200, 300, 600, -100], [270000, 40000, -90000], [432000000, -200000000], [313600000000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 313600000000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 25000000 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (311.26867614926164 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √25000000 ≤ 5000.00001 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √25000000 ≤ 192.01 := by
