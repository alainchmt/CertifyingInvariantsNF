...
import IdealArithmetic.Examples.NF3_3_109384_1.PrimesBelowCert3_3_109384_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 51*X + 79`.
...
def SturmRC : SturmBuilderOfList [[79, -51, -1, 1], [-51, -2, 3], [-660, 308], [437536]] [79, -51, -1, 1] [-51, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[79, -51, -1, 1], [-51, -2, 3], [-660, 308], [437536]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (73.50611562760089 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √109384 ≤ 330.73253 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √109384 ≤ 73.506118 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 74} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB74

def 𝔭' : Fin 24 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB74.β i ∈ List.ofFn PB74.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB74.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB74) (hc ▸ this) ))
