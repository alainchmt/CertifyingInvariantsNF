...
import IdealArithmetic.Examples.NF3_1_217892_1.PrimesBelowCert3_1_217892_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 68*X + 472`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/6*a^2 - 1/2*a + 1/3]` -/
...
def SturmRC : SturmBuilderOfList [[472, 68, -1, 1], [68, -2, 3], [-4316, -406], [-7844112]] [472, 68, -1, 1] [68, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[472, 68, -1, 1], [68, -2, 3], [-4316, -406], [-7844112]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (132.0842786781461 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √217892 ≤ 466.789042 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √217892 ≤ 103.740898 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 133} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB133

def 𝔭' : Fin 38 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB133.β i ∈ List.ofFn PB133.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB133.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB133) (hc ▸ this) ))
