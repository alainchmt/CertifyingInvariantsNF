...
import IdealArithmetic.Examples.NF3_3_50172_1.PrimesBelowCert3_3_50172_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 48*X + 18`.
...
def SturmRC : SturmBuilderOfList [[18, -48, -1, 1], [-48, -2, 3], [-114, 290], [451548]] [18, -48, -1, 1] [-48, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[18, -48, -1, 1], [-48, -2, 3], [-114, 290], [451548]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (49.78579361124873 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √50172 ≤ 223.991081 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √50172 ≤ 49.785796 := by
