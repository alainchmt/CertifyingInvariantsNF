...
import IdealArithmetic.Examples.NF3_1_439803_1.PrimesBelowCert3_1_439803_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 165*X - 614`.
...
def SturmRC : SturmBuilderOfList [[-614, 165, 0, 1], [165, 0, 3], [5526, -990], [-28147392]] [-614, 165, 0, 1] [165, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-614, 165, 0, 1], [165, 0, 3], [5526, -990], [-28147392]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -28147392 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -439803 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (187.65055057689648 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √439803 ≤ 663.176457 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √439803 ≤ 147.382546 := by
