...
import IdealArithmetic.Examples.NF3_1_664563_1.PrimesBelowCert3_1_664563_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 66*X - 423`.
...
def SturmRC : SturmBuilderOfList [[-423, 66, 0, 1], [66, 0, 3], [3807, -396], [-5981067]] [-423, 66, 0, 1] [66, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-423, 66, 0, 1], [66, 0, 3], [3807, -396], [-5981067]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (230.66649262923872 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √664563 ≤ 815.207346 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √664563 ≤ 181.167188 := by
