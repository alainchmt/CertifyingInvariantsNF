...
import IdealArithmetic.Examples.NF3_1_743372_1.PrimesBelowCert3_1_743372_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 61*X + 1033`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/6*a^2 - 1/6]` -/
...
def SturmRC : SturmBuilderOfList [[1033, -61, -1, 1], [-61, -2, 3], [-9236, 368], [-26761392]] [1033, -61, -1, 1] [-61, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[1033, -61, -1, 1], [-61, -2, 3], [-9236, 368], [-26761392]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -26761392 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -743372 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (243.95993362332004 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √743372 ≤ 862.190244 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √743372 ≤ 191.607832 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 244} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB244

def 𝔭' : Fin 58 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB244.β i ∈ List.ofFn PB244.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB244.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB244) (hc ▸ this) ))
