...
import IdealArithmetic.Examples.NF3_1_311116_1.PrimesBelowCert3_1_311116_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 31*X - 651`.
...
def SturmRC : SturmBuilderOfList [[-651, 31, -1, 1], [31, -2, 3], [5828, -184], [-11200176]] [-651, 31, -1, 1] [31, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-651, 31, -1, 1], [31, -2, 3], [5828, -184], [-11200176]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -11200176 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -311116 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (157.8288149951542 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √311116 ≤ 557.777744 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √311116 ≤ 123.96061 := by
