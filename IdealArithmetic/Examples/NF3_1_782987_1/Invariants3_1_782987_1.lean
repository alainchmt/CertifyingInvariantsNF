...
import IdealArithmetic.Examples.NF3_1_782987_1.PrimesBelowCert3_1_782987_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 43*X + 118`.
...
def SturmRC : SturmBuilderOfList [[118, 43, -1, 1], [43, -2, 3], [-1105, -256], [-782987]] [118, 43, -1, 1] [43, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[118, 43, -1, 1], [43, -2, 3], [-1105, -256], [-782987]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (250.37572891583233 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √782987 ≤ 884.865537 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √782987 ≤ 196.646786 := by
