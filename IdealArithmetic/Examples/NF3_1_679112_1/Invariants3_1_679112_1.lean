...
import IdealArithmetic.Examples.NF3_1_679112_1.PrimesBelowCert3_1_679112_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 2*X + 158`.
...
def SturmRC : SturmBuilderOfList [[158, 2, -1, 1], [2, -2, 3], [-1424, -10], [-679112]] [158, 2, -1, 1] [2, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[158, 2, -1, 1], [2, -2, 3], [-1424, -10], [-679112]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (233.17765617045166 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √679112 ≤ 824.08253 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √679112 ≤ 183.139451 := by
