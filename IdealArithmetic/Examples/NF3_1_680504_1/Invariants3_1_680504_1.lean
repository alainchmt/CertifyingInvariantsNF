...
import IdealArithmetic.Examples.NF3_1_680504_1.PrimesBelowCert3_1_680504_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 169*X + 387`.
...
def SturmRC : SturmBuilderOfList [[387, 169, -1, 1], [169, -2, 3], [-3652, -1012], [-24498144]] [387, 169, -1, 1] [169, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[387, 169, -1, 1], [169, -2, 3], [-3652, -1012], [-24498144]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -24498144 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -680504 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (233.4164998569723 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √680504 ≤ 824.9266729999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √680504 ≤ 183.327038 := by
