...
import IdealArithmetic.Examples.NF3_1_628963_1.PrimesBelowCert3_1_628963_1
...
/- Number field `K(α)` with α root of polynomial `X^3 + 28*X - 761`.
...
def SturmRC : SturmBuilderOfList [[-761, 28, 0, 1], [28, 0, 3], [6849, -168], [-15724075]] [-761, 28, 0, 1] [28, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-761, 28, 0, 1], [28, 0, 3], [6849, -168], [-15724075]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -15724075 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -628963 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (224.4034386527566 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √628963 ≤ 793.071886 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √628963 ≤ 176.248197 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 225} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB225

def 𝔭' : Fin 49 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB225.β i ∈ List.ofFn PB225.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB225.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB225) (hc ▸ this) ))
