...
import IdealArithmetic.Examples.NF3_1_702351_1.PrimesBelowCert3_1_702351_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 3*X - 1129`.
...
def SturmRC : SturmBuilderOfList [[-1129, -3, 0, 1], [-3, 0, 3], [10161, 18], [-34415199]] [-1129, -3, 0, 1] [-3, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-1129, -3, 0, 1], [-3, 0, 3], [10161, 18], [-34415199]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -34415199 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -702351 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (237.13356048105697 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √702351 ≤ 838.06385 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √702351 ≤ 186.246411 := by
