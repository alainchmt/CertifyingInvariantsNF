import IdealArithmetic.Examples.NF3_1_42423_4.ClassGroupSaturated3_1_42423_4_3
import IdealArithmetic.Examples.NF3_1_42423_4.RelationIdeals3_1_42423_4F0
import IdealArithmetic.Examples.NF3_1_42423_4.RelationIdeals3_1_42423_4F1
import IdealArithmetic.Examples.NF3_1_42423_4.PrimesBelowCert3_1_42423_4
...
/- Number field `K(α)` with α root of polynomial `X^3 + 21*X - 113`.
Class group structure `[3]`, generated by class of ideals `[(3, 1/3*a^2 + 2/3*a + 16/3)]`. -/
...
def SturmRC : SturmBuilderOfList [[-113, 21, 0, 1], [21, 0, 3], [1017, -126], [-381807]] [-113, 21, 0, 1] [21, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-113, 21, 0, 1], [21, 0, 3], [1017, -126], [-381807]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (58.28714846754483 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √42423 ≤ 205.968454 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √42423 ≤ 45.780768 := by
...
def BM : (i : Fin 2) → Fin (e i) → (Fin 1 → ℕ) := by 
  rintro ⟨i, hi⟩ 
  interval_cases i 
  exact ![![0], ![1], ![1], ![0], ![0], ![0], ![1], ![2], ![1], ![1], ![1], ![1], ![2], ![0], ![0], ![1], ![2]]
  exact ![![2], ![2], ![1]]

lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 59} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB59

def 𝔭' : Fin 20 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB59.β i ∈ List.ofFn PB59.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB59.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB59) (hc ▸ this) ))
...
lemma relations_proof0 (i : Fin 17) : IsInClass O J (𝔭 0 i) (BM 0 i) := by
  show IsInClass O J (𝔭 0 i) (![![0], ![1], ![1], ![0], ![0], ![0], ![1], ![2], ![1], ![1], ![1], ![1], ![2], ![0], ![0], ![1], ![2]] i)
  unfold IsInClass
...
lemma relations_proof1 (i : Fin 3) : IsInClass O J (𝔭 1 i) (BM 1 i) := by
  show IsInClass O J (𝔭 1 i) (![![2], ![2], ![1]] i)
  unfold IsInClass
  simp only [Fin.isValue, Fin.prod_univ_castSucc, Finset.univ_eq_empty, Finset.prod_empty, Fin.succ_zero_eq_one, one_mul]
  fin_cases i
