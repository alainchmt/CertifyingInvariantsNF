...
import IdealArithmetic.Examples.NF3_1_400040_1.PrimesBelowCert3_1_400040_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 48*X - 1208`.
...
def SturmRC : SturmBuilderOfList [[-1208, -48, -1, 1], [-48, -2, 3], [10920, 290], [-40004000]] [-1208, -48, -1, 1] [-48, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-1208, -48, -1, 1], [-48, -2, 3], [10920, 290], [-40004000]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -40004000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -400040 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (178.96725689665385 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √400040 ≤ 632.487164 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √400040 ≤ 140.562703 := by
