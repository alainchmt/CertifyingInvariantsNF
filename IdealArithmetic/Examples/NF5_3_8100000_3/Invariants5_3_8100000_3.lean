...
import IdealArithmetic.Examples.NF5_3_8100000_3.PrimesBelowCert5_3_8100000_3
...
/- Number field `K(α)` with α root of polynomial `X^5 - 45*X - 36`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, a^2, 1/6*a^3 - 1/2*a, 1/12*a^4 - 1/12*a^3 - 1/4*a^2 + 1/4*a]` -/
...
def SturmRC : SturmBuilderOfList [[-36, -45, 0, 0, 0, 1], [-45, 0, 0, 0, 5], [900, 900], [41990400000]] [-36, -45, 0, 0, 0, 1] [-45, 0, 0, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [-3645000000, 3645000000, -3645000000, 3645000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-36, -45, 0, 0, 0, 1], [-45, 0, 0, 0, 5], [900, 900], [41990400000]]
  SB := SturmRC
  k := 3
  r := 4
  hr := by decide
...
lemma T_discr : T.discr = -41990400000 :=  by
...
theorem K_minowski : minkowskiBoundFB K ≤ (139.16020562648578 : ℝ) := by
...
lemma relations_proof0 (i : Fin 8) : IsInClass O J (𝔭 0 i) (BM 0 i) := by
  show IsInClass O J (𝔭 0 i) (![![0], ![0], ![0], ![0], ![0], ![0], ![0], ![0]] i)
  unfold IsInClass
  fin_cases i
