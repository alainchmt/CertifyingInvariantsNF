...
import IdealArithmetic.Examples.NF5_1_13500000_3.PrimesBelowCert5_1_13500000_3
...
/- Number field `K(α)` with α root of polynomial `X^5 - 5*X^3 - 10*X^2 + 80`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, a^2, 1/2*a^3 - 1/2*a, 1/28*a^4 - 1/7*a^3 + 11/28*a^2 + 1/14*a - 2/7]` -/
...
def SturmRC : SturmBuilderOfList [[80, 0, -10, -5, 0, 1], [0, -20, -15, 0, 5], [-2000, 0, 150, 50], [60000, -18000, -3000], [18000000, -6840000], [42336000000]] [80, 0, -10, -5, 0, 1] [0, -20, -15, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 2500, 9000000, 46785600000000]
  f := [1, 25, 2500, 9000000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [-750, 250], [450000, -150000], [177120000000, 20520000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[80, 0, -10, -5, 0, 1], [0, -20, -15, 0, 5], [-2000, 0, 150, 50], [60000, -18000, -3000], [18000000, -6840000], [42336000000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 42336000000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 13500000 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (228.73748037396624 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √13500000 ≤ 3674.234624 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √13500000 ≤ 141.10061 := by
