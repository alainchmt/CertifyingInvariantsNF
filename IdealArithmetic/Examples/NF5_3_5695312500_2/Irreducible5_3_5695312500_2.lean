...
def P13PI0 : CertificateIrreducibleZModOfList' 13 2 2 3 [10, 5, 1] where
 m := 1
 P := ![2]
 exp := ![1]
 hneq := by decide
...
def P13PI1 : CertificateIrreducibleZModOfList' 13 3 2 3 [7, 5, 8, 1] where
 m := 1
 P := ![3]
 exp := ![1]
 hneq := by decide
...
def P19PI1 : CertificateIrreducibleZModOfList' 19 4 2 4 [1, 1, 10, 3, 1] where
 m := 1
 P := ![2]
 exp := ![2]
 hneq := by decide
...
noncomputable def C : IrreducibleCertificateIntPolynomial T l where
 ...
 hirr := by
  ...
  · exact irreducible_ofList_of_linear (R := ZMod 19) _ (by decide) rfl
