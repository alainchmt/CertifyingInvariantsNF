...
import IdealArithmetic.Examples.NF5_1_7812500_1.PrimesBelowCert5_1_7812500_1
...
/- Number field `K(α)` with α root of polynomial `X^5 + 25*X - 10`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, a^2, a^3, 1/18*a^4 + 5/18*a^3 + 7/18*a^2 - 1/18*a + 1/9]` -/
...
def SturmRC : SturmBuilderOfList [[-10, 25, 0, 0, 0, 1], [25, 0, 0, 0, 5], [250, -500], [-2531250000]] [-10, 25, 0, 0, 0, 1] [25, 0, 0, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [-78125000, -156250000, -312500000, -625000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-10, 25, 0, 0, 0, 1], [25, 0, 0, 0, 5], [250, -500], [-2531250000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 2531250000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 7812500 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (174.00888961408538 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √7812500 ≤ 2795.0849820000003 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √7812500 ≤ 107.341263 := by
