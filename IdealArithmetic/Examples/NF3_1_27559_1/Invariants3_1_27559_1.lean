...
import IdealArithmetic.Examples.NF3_1_27559_1.PrimesBelowCert3_1_27559_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 + 30*X + 63`.
...
def SturmRC : SturmBuilderOfList [[63, 30, -1, 1], [30, -2, 3], [-597, -178], [-248031]] [63, 30, -1, 1] [30, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[63, 30, -1, 1], [30, -2, 3], [-597, -178], [-248031]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (46.98094873100516 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √27559 ≤ 166.009046 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √27559 ≤ 36.900899 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 47} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB47

def 𝔭' : Fin 11 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB47.β i ∈ List.ofFn PB47.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB47.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB47) (hc ▸ this) ))
