...
import IdealArithmetic.Examples.NF5_1_2700000_1.PrimesBelowCert5_1_2700000_1
...
/- Number field `K(α)` with α root of polynomial `X^5 - 5*X^3 - 10*X^2 + 30*X + 20`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, a^2, 1/2*a^3 - 1/2*a^2, 1/24*a^4 - 5/24*a^3 - 1/6*a^2 + 5/12*a + 1/6]` -/
...
def SturmRC : SturmBuilderOfList [[20, 30, -10, -5, 0, 1], [30, -20, -15, 0, 5], [-500, -600, 150, 50], [12000, 15000, -9000], [6120000, 4680000], [6220800000]] [20, 30, -10, -5, 0, 1] [30, -20, -15, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  e := [25, 2500, 81000000, 21902400000000]
  f := [1, 25, 2500, 81000000]
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [-750, 250], [-2100000, -450000], [125280000000, -42120000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[20, 30, -10, -5, 0, 1], [30, -20, -15, 0, 5], [-500, -600, 150, 50], [12000, 15000, -9000], [6120000, 4680000], [6220800000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 6220800000 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 2700000 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (102.3000388876875 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √2700000 ≤ 1643.167683 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √2700000 ≤ 63.107639 := by
