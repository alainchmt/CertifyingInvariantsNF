...
import IdealArithmetic.Examples.NF5_1_1000000_1.PrimesBelowCert5_1_1000000_1
...
/- Number field `K(α)` with α root of polynomial `X^5 - 5*X - 12`.
...
def SturmRC : SturmBuilderOfList [[-12, -5, 0, 0, 0, 1], [-5, 0, 0, 0, 5], [300, 100], [-64000000]] [-12, -5, 0, 0, 0, 1] [-5, 0, 0, 0, 5] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
  ...
  epos := by decide
  fpos := by decide
  Q := [[0, 5], [-135000000, 45000000, -15000000, 5000000]]
  hel := by decide
  hfl := by decide
  hQl := by decide
  hrem := by
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-12, -5, 0, 0, 0, 1], [-5, 0, 0, 0, 5], [300, 100], [-64000000]]
  SB := SturmRC
  k := 1
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 64000000 :=  by
...
theorem K_minowski : minkowskiBoundFB K ≤ (62.26173522985233 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √1000000 ≤ 1000.00001 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.0384 * √1000000 ≤ 38.41 := by
