...
import IdealArithmetic.Examples.NF3_1_447535_1.PrimesBelowCert3_1_447535_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 56*X - 1024`.
Class group structure `[]`, generated by class of ideals `[]`. -/

/- Ring of integers with basis `[1, a, 1/8*a^2 - 1/8*a]` -/
...
def SturmRC : SturmBuilderOfList [[-1024, -56, -1, 1], [-56, -2, 3], [9272, 338], [-28642240]] [-1024, -56, -1, 1] [-56, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-1024, -56, -1, 1], [-56, -2, 3], [9272, 338], [-28642240]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
lemma T_discr : T.discr = -28642240 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = -447535 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (189.2927807761955 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √447535 ≤ 668.9805779999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √447535 ≤ 148.672351 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 190} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB190

def 𝔭' : Fin 45 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB190.β i ∈ List.ofFn PB190.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB190.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB190) (hc ▸ this) ))
