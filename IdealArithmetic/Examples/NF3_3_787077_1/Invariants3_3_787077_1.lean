...
import IdealArithmetic.Examples.NF3_3_787077_1.PrimesBelowCert3_3_787077_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - 243*X - 510`.
...
def SturmRC : SturmBuilderOfList [[-510, -243, 0, 1], [-243, 0, 3], [4590, 1458], [50372928]] [-510, -243, 0, 1] [-243, 0, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[-510, -243, 0, 1], [-243, 0, 3], [4590, 1458], [50372928]]
  SB := SturmRC
  k := 3
  r := 3
  hr := by decide
...
lemma T_discr : T.discr = 50372928 :=  by
  convert discriminant_eq_DiscriminantOfPRemainder_of_SturmBuilderOfList SturmRC
  rw [T_ofList]

theorem K_discr : NumberField.discr K = 787077 := by
...
theorem K_minowski : minkowskiBoundFB K ≤ (197.15968932260583 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √787077 ≤ 887.1736119999999 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √787077 ≤ 197.159692 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 198} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB198

def 𝔭' : Fin 45 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB198.β i ∈ List.ofFn PB198.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB198.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB198) (hc ▸ this) ))
