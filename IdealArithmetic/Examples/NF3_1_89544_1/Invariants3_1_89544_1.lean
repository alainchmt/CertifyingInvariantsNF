...
import IdealArithmetic.Examples.NF3_1_89544_1.PrimesBelowCert3_1_89544_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 14*X + 66`.
...
def SturmRC : SturmBuilderOfList [[66, -14, -1, 1], [-14, -2, 3], [-580, 86], [-89544]] [66, -14, -1, 1] [-14, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[66, -14, -1, 1], [-14, -2, 3], [-580, 86], [-89544]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (84.67732723250974 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √89544 ≤ 299.239045 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √89544 ≤ 66.507566 := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 85} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB85

def 𝔭' : Fin 25 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB85.β i ∈ List.ofFn PB85.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB85.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB85) (hc ▸ this) ))
