...
theorem K_minowski : minkowskiBoundFB K ≤ (9.685782846132922 : ℝ) := by
...
lemma primesGenerationBelow : {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < 10} ⊆ Set.range (Fin.addCasesIter e 𝔭) := by
  rw [← cert_eq_𝔭]
  refine le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq (Subalgebra.equivOfEq _ _ O_integral_closure).toRingEquiv PB10

def 𝔭' : Fin 6 → nonZeroDivisors (Ideal O) := by
  intro i
  have : PB10.β i ∈ List.ofFn PB10.β := by
    simp only [List.mem_ofFn, exists_apply_eq_apply]
  refine Ideal.toNonZeroDivisorOfNeZero ((PB10.β) i)
    (fun hc => ((zero_ne_mem_of_PrimesBelowCertificate_Fn _ PB10) (hc ▸ this) ))
...
lemma relations_proof0 (i : Fin 6) : IsInClass O J (𝔭 0 i) (BM 0 i) := by
  show IsInClass O J (𝔭 0 i) (![![5, 0], ![1, 0], ![3, 1], ![1, 1], ![5, 1], ![0, 1]] i)
  unfold IsInClass
