...
import IdealArithmetic.Examples.NF3_1_192584_1.PrimesBelowCert3_1_192584_1
...
/- Number field `K(α)` with α root of polynomial `X^3 - X^2 - 35*X + 199`.
...
def SturmRC : SturmBuilderOfList [[199, -35, -1, 1], [-35, -2, 3], [-1756, 212], [-770336]] [199, -35, -1, 1] [-35, -2, 3] where
  hlen := by decide
  h0 := by decide
  h1 := by decide
  hlast := by decide
...
def RC : RankUnitsCertificate O where
  ...
  P := [[199, -35, -1, 1], [-35, -2, 3], [-1756, 212], [-770336]]
  SB := SturmRC
  k := 1
  r := 2
  hr := by decide
...
theorem K_minowski : minkowskiBoundFB K ≤ (124.17743400990149 : ℝ) := by
  refine K_minkowski_decimal _ ?_
  erw [K_nrComplexPlaces, K_discr, K_finrank]
  have hraux: √192584 ≤ 438.84394599999996 := by
      refine Real.sqrt_le_iff.mpr ?_
      norm_num
  have : 0.2222222222222222 * √192584 ≤ 97.530877 := by
