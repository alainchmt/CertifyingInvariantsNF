...
structure PrimeIdeal (O : Type*) (p : ℕ)  [CommRing O]  where
  r : ℕ
  n : ℕ
  hpos : 0 < n
...
lemma PrimeIdeal_polynomial_degree  {O : Type*} {p : ℕ} [CommRing O]
...
noncomputable def isAdjoinRoot_quot_ofPrimeIdeal {O : Type*} {p : ℕ}
    [Fact $ Nat.Prime p] [CommRing O] (I : PrimeIdeal O p) [Algebra (ZMod p) (O ⧸ I.I)] :
...
noncomputable def PrimeIdeal_residue_field_is_field {O : Type*} {p : ℕ}
    [Fact $ Nat.Prime p] [CommRing O] (I : PrimeIdeal O p) : IsField (O ⧸ I.I) := by
  have hpmem : ↑p ∈ I.I := by
    have := I.hpmem
...
/-- The underlying ideal of `I : PrimeIdeal p O` is a prime ideal. -/
