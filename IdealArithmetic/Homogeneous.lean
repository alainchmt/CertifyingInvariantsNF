...
theorem MvPolynomial.coeff_aeval_X_pow_mul' {σ : Type*} {w : σ → ℕ} {j : σ} (p : MvPolynomial σ R)
  ...
  · obtain ⟨⟨d', hdeg⟩, hdiv⟩ := hdeg
    ...
    by_cases hij : i = j
    · subst hij
      rw [Finsupp.weight_sub _ _ (Finsupp.single_le_iff.mpr hdiv), Finsupp.weight_single, hdeg, smul_eq_mul,
  ...
  case _ a b p _ _ ih =>
    ...
    · use Finsupp.weight w a
      simp [add_mul, add_comm, mul_comm, Finsupp.weight_single]
    · rw [map_add, Finsupp.weight_single, smul_eq_mul, Finsupp.coe_add, Pi.add_apply, Finsupp.single_eq_same,
...
lemma MvPolynomial.isWeightedHomogeneous_mul_X_pow_iff {σ : Type*} {w : σ → ℕ}
  ...
  · intro hp
    ...
    · rw [← this, Finsupp.weight_add, Finsupp.weight_single, smul_eq_mul, Nat.add_sub_cancel_left]
...
lemma MvPolynomial.isWeightedHomogeneous_mul_X_iff {σ : Type*} {w : σ → ℕ} {p : MvPolynomial σ R} {i : σ}
  ...
  · intro hp
    have hn0 : w i ≤ n := by
      ...
      rwa [Finsupp.weight_add, Finsupp.weight_single, smul_eq_mul, one_mul] at hp
    ...
    simpa [add_comm, Finsupp.weight_single] using this
...
lemma MvPolynomial.isHomogeneous_mul_X_iff {σ : Type*} {p : MvPolynomial σ R} {i : σ}
  ...
  · intro hp
    ...
    intro d hd
    have := @hp (Finsupp.single i 1 + d) ?_
    simpa [add_comm 1, Finsupp.weight_single] using this
    · rwa [add_comm, coeff_mul_X]
  · rintro ⟨m, hp, rfl⟩
