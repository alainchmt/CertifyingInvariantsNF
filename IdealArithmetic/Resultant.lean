...
import IdealArithmetic.DegreeLT
import IdealArithmetic.Homogeneous
import IdealArithmetic.PolynomialsAsLists
import IdealArithmetic.SylvesterMatrix
...
/- Delete: @[simp] lemma Fin.castSucc_ne_last {n : ℕ} (i : Fin n) : i.castSucc ≠ Fin.last n := by
...
lemma lt_of_sum_eq_of_ne {M : Type*} [AddCommMonoid M] [PartialOrder M]
    [IsOrderedCancelAddMonoid M] {n : M}
...
-- Delete : @[simp] lemma Fin.mk_zero' [NeZero m] (h : 0 < m) : (⟨0, h⟩ : Fin m) = 0 := rfl
...
lemma MvPolynomial.X_sub_X_dvd_of_rename_eq_zero [IsDomain R]
  ...
  · rw [hij, Function.update_eq_self_iff.mpr, rename_id] at h
    simp only [AlgHom.coe_id, id_eq] at h
    rw [h]
    apply dvd_zero
    · rfl
  ...
  congr
  · ext x
    simp only [ne_eq, AlgHom.toRingHom_eq_coe, RingHom.coe_comp, RingHom.coe_coe, coe_evalRingHom,
      Function.comp_apply, coeff_C]
    conv =>
      left ; right ; right
      rw [Polynomial.algebraMap_apply, algebraMap_eq]
    rw [Polynomial.eval_C, MvPolynomial.rename_C, coeff_C]
  · ext a
    by_cases hai : a = i
    · simp [hai, ei]
    simp [e_ne_i, hai]
    congr
    rw [Polynomial.eval_C, rename_X]
...
lemma MvPolynomial.irreducible_X_sub_X [IsDomain R] {σ : Type*} {i j : σ} (hij : i ≠ j) :
  ...
  simpa [e, hij.symm, optionEquivLeft] using irreducible_X_sub_C _
...
lemma resultant_eq_zero_iff {P Q : K[X]} :
  ...
  · rintro ⟨ab, hfab, hab0⟩
    ...
    obtain ⟨a, b⟩ := ab
    refine ⟨b, a, ?_, ?_⟩
    · simpa [-not_and, Classical.not_and_iff_not_or_not, or_comm] using hab0
...
lemma sylvesterMatrixVec_row_zero {m n : ℕ} (f : Fin (m + 1 + 1) → R) (g : Fin (n + 1 + 1) → R)
  ...
  · simp only [Nat.succ_eq_add_one, transpose_apply, of_apply, Fin.addCases_right, Fin.cast_zero,
      Fin.coe_natAdd, add_eq_zero, one_ne_zero, and_false, false_and, add_eq_left, ↓reduceIte]
    split_ifs with h
    · have : j = 0 := by ext; exact h
      simp only [this, sylvesterVec_zero]
      exact Fin.pad_of_lt _ _ _ _
...
lemma List.finset_prod_get (a : List ι) (f : ι → R) :
  ...
  refine (Finset.prod_map_toList _ _).symm.trans (List.Perm.prod_eq ?_)
