...
lemma subgroup_closure_eq_classGroup' {m n : Type} {D : ℝ} {b : ℕ}
  ...
  · rw [Subgroup.map_top_of_surjective _ (by exact EquivLike.surjective E)]
    ...
    convert subgroup_closure_eq_classGroup_bound' (K := K) (m := m) (n := n)
       (b := b) ( 𝔭 := gg) (J := xx) (𝔭' := gg') (J' := xx') ?_ ?_ hbg hB ?_ M ?_
    · rw [← Set.range_comp]
    ...
    · intro i
      simp only [hx', xx, xx']
    · convert primes_below_bound_of_equiv φ.symm (𝔭 := gg) b ?_
      convert hg
      ext i
      simp only [Function.comp_apply, Ideal.map_symm, Ideal.mem_comap, Ideal.apply_mem_of_equiv_iff,
        gg]
...
structure PrimesBelowBoundCertificate (S : Type*) [CommRing S] [Nontrivial S] (B : ℕ) where
  ...
  β : Fin r → Ideal S
  Il : Fin m → List (Ideal S) /-- Lists of ideals over the `i`-th prime, with norm less than `B`-/
...
lemma le_primes_below_bound_of_PrimesBelowBoundCertificate_le_of_eq {O : Type*} [CommRing O]
    ...
    (A : PrimesBelowBoundCertificate O B) :
    {I : Ideal O | 0 < I.absNorm ∧ I.IsPrime ∧ I.absNorm < B} ⊆ Set.range (A.β) := by
