...
theorem blockSign_generates_all :
  ...
  -- The `p = 13` certificate's factor degrees are `{1,1,1,1,2}`.
  have hM : cert13.factors.map Polynomial.natDegree = ({1, 1, 1, 1, 2} : Multiset ℕ) := by
  ...
  set σ' : ((g.map (Int.castRingHom ℚ)).comp (X ^ 2)).Gal :=
    EvenSextic.galCongr fℚ_eq_gℚ_comp σf with hσ'def
  ...
  have hpartsF : (EvenSextic.fRootEquiv (g.map (Int.castRingHom ℚ)) σ').partition.parts
    ...
    ((EvenSextic.galActionHom_partition_parts_eq (g.map (Int.castRingHom ℚ)) σ').symm.trans
      h1).trans hM
  -- Cycle type `{2}`: a single transposition of one `±`-pair.
  ...
  exact fun θ => EvenSextic.blockSign_generates (g.map (Int.castRingHom ℚ))
    X_sq_natDegree_ne_zero gℚ_monic gℚ_irreducible hσ'mem hθ₀ θ
...
theorem gal_f_mulEquiv :
  ...
  obtain ⟨C, hC⟩ : ∃ C : Subgroup GalP, K.IsComplement' C := by
    ...
    rw [card_ker, show K.index = 3 by
      have h1 : Nat.card K * K.index = Nat.card GalP := K.card_mul_index
      rw [card_ker, cardGalP] at h1; omega]
    decide
  have hCcard : Nat.card C = 3 := by have := hC.card_mul; rw [card_ker, cardGalP] at this; omega
  ...
  have hτ : orderOf τ = 3 := by
    have hhr : τ = Gal.galActionHom gq (gq.comp (X ^ 2)).SplittingField (rc (c₀ : GalP)) := by
      apply Equiv.ext; intro θ
      exact (galActionHom_restrictComp_eq_hRoot gq X_sq_natDegree_ne_zero (c₀ : GalP) θ).symm
    rw [hhr, orderOf_injective _ (Gal.galActionHom_injective gq _)]
    have h2 : orderOf (rc (c₀ : GalP)) = orderOf c₀ :=
      orderOf_injective (rc.comp C.subtype) hinjC c₀
    rw [h2]
    have hdvd : orderOf c₀ ∣ 3 := hCcard ▸ orderOf_dvd_natCard c₀
  ...
  -- **Base case.** On the block-sign coordinates, conjugation by `c₀` is precomposition by `τ`
  -- (`blockSign_conj`): `blockSignEquiv (c₀ κ c₀⁻¹) = precompAut τ (blockSignEquiv κ)`.
  have base : ∀ κ : K,
      blockSignEquiv gq X_sq_natDegree_ne_zero gℚ_monic root_ne_neg blockSign_generates_all (nAct c₀ κ)
    ...
    show Multiplicative.ofAdd (blockSign gq gℚ_monic (↑(nAct c₀ κ)))
        = Multiplicative.ofAdd ((blockSign gq gℚ_monic (↑κ)) ∘ ⇑τ.symm)
    congr 1; funext θ
    show blockSign gq gℚ_monic (↑(nAct c₀ κ)) θ = blockSign gq gℚ_monic (↑κ) (τ.symm θ)
    have hsymm : τ.symm θ = hRoot gq (↑c₀ : GalP)⁻¹ θ := by rw [hτdef]; exact rfl
    have hcoe : (↑(nAct c₀ κ) : GalP) = ↑c₀ * ↑κ * (↑c₀)⁻¹ := by
  ...
  -- **Full equivariance.** Power up the base case through `C ≅ C₃` (`conj_pow_trans`): the
  -- `blockSignEquiv`/`fG` pair intertwines the conjugation action `nAct` with `blockAction τ`.
  have compat : ∀ c : C, (nAct c).trans (blockSignEquiv gq X_sq_natDegree_ne_zero gℚ_monic
    ...
    have hnact : nAct c = (nAct c₀) ^ (Multiplicative.toAdd (fG c)).val := by
      conv_lhs => rw [← hpow]
      rw [map_pow]
    ...
    rw [hnact, hblk, hgenb]
    exact MulEquiv.ext
      (conj_pow_trans _ (nAct c₀) (precompAut τ) base (Multiplicative.toAdd (fG c)).val)
