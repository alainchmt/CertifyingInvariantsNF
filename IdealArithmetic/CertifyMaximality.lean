import IdealArithmetic.CertifyKernel
import IdealArithmetic.DedekindCriteria
import IdealArithmetic.Semilinear
...
lemma iter_Frob (R : Type*) (p : ℕ) [CommRing R]
  ...
  · simp only [pow_zero, Function.iterate_zero] ; rfl
  · simp only [pow_add, pow_one, Function.iterate_succ, ← hn] ; rfl
...
lemma basis_radical_of_linear_independent_ker_im_in_repr [NoZeroSMulDivisors ℤ O]
  ...
  have aux4 : Finset.univ.sum (λ i => (↑(s i  : ZMod p) • (⟨b i, aux1 i ⟩ mod p ))) +
    ...
    Sum.elim_inr]
    congr
    · simp only [basis_radical_of_linear_independent_ker_im, id_eq, eq_mpr_eq_cast, cast_eq,
        basis_radical_def_left]
    · simp only [basis_radical_of_linear_independent_ker_im, id_eq, eq_mpr_eq_cast, cast_eq,
        Pi.mul_apply, basis_radical_def_right]
      simp only [zsmul_eq_mul, Int.cast_natCast]
  rw [aux3, aux4]
  symm
