...
theorem Finsupp.apply_linearCombination' (f : M →ₛₗ[g] N) (v) (l : α →₀ R) :
  ...
  induction' l using Finsupp.induction_linear with x y hx hy
  --apply Finsupp.induction_linear l
  · simp only [map_zero, mapRange_zero]
  · rw [map_add, map_add, hx, hy, mapRange_add, map_add]
    apply map_add
  · simp only [linearCombination_single, LinearMap.map_smulₛₗ, mapRange_single,
    Function.comp_apply]
